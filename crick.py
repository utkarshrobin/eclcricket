...
MEDIA = {
    ...
    0: 'https://res.cloudinary.com/dxgfxfoog/video/upload/v1777717596/VID_20260502_155429_102_xtppvn.mp4', # Updated defense GIF
...
def set_afk_timer(context, chat_id, user_id, role):
    ...
    if game.get('mode') == 'TEAM':
        ...
        context.job_queue.run_once(team_afk_warning_30, 30, data={'chat_id': chat_id, 'user_id': user_id, 'role': role}, name=f"afk60_{chat_id}") # keeping prefix for clear_afk compatibility
                ...
                if game and game['state'] == 'PLAYING' and game.get('waiting_for') == 'BOWLER':
                    ...
                    if bowler and update.effective_user.id == bowler['id']:
                        ...
                        await update.message.reply_text(
                            f"🥎 <b>Your Turn to Bowl!</b>\nType {allowed_vals} or use your Special Delivery! 🤔👇", 
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_user_playing_anywhere(context, target_user.id):
        await update.message.reply_text(f"❌ {target_user.first_name} is already playing in another active game!")
        return
...
async def remove_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if removed:
        await update.message.reply_text(f"✅ <b>{target_name}</b> has been successfully removed from their team! 🚪", parse_mode='HTML')
    else:
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        ...
        if is_user_playing_anywhere(context, user_id):
            await query.answer("You are already playing/hosting in another game!", show_alert=True)
            return
        ...
        if is_user_playing_anywhere(context, user_id):
            await context.bot.send_message(chat_id, f"❌ {update.effective_user.first_name}, you are already in a team or playing somewhere else!")
            return
        ...
        if roll <= 60: # 60% WIDE
            ...
            if is_free_hit:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    if game['batting_team_ref']['wickets'] >= len(game['batting_team_ref']['players']) - 1:
                        ...
                        game['waiting_for'] = 'TEAM_BATTER_SELECT'
                        await context.bot.send_message(group_id, f"🏏 Captain/Host, type <code>/batting</code> to view available batters or <code>/batting [number]</code> to select the next batter.", parse_mode='HTML')
                else:
                    game['batter_idx'] += 1
            ...
            if is_over_complete:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    spell_text += f"Bowling Captain/Host, select next bowler using <code>/bowling [num]</code>."
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler('score', matchscore_command))
