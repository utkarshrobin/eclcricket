...
MEDIA = {
    ...
    0: 'https://res.cloudinary.com/dxgfxfoog/video/upload/v1777688554/VID_20260502075045_eab7uu.mp4', 
...
SCOREBOARD_IMG = 'https://res.cloudinary.com/dxgfxfoog/image/upload/v1777690370/file_000000003b007208989acb9335cd9f20_skv1se.png'
...
def generate_team_scorecard(game):
    ...
    for team_key, team_name in [('team_a', 'Team A 🔴'), ('team_b', 'Team B 🔵')]:
        ...
        overs = team['balls_bowled'] // 6
        balls = team['balls_bowled'] % 6
        text += f"<b>{team_name}</b> - {team['score']}/{team['wickets']} ({overs}.{balls} Ov)\n"
    ...
    if game['state'] == 'TEAM_FINISHED':
        if game['team_a']['score'] > game['team_b']['score']:
            text += "🎉 <b>TEAM A WINS!</b> 🎉\n"
        elif game['team_b']['score'] > game['team_a']['score']:
            text += "🎉 <b>TEAM B WINS!</b> 🎉\n"
        else:
            text += "🤝 <b>IT\'S A TIE!</b> 🤝\n"
...
def generate_teams_message(game):
    ...
    text += "🔴 <b>TEAM A</b>\n"
    for i, p in enumerate(game['team_a']['players'], 1):
        cap = " (C) 👑" if game['team_a'].get('captain') == p['id'] else ""
        text += f" {p.get('num', i)}. <a href='tg://user?id={p['id']}'>{p['name']}</a>{cap}\n"
        
    text += "\n🔵 <b>TEAM B</b>\n"
    for i, p in enumerate(game['team_b']['players'], 1):
        cap = " (C) 👑" if game['team_b'].get('captain') == p['id'] else ""
        text += f" {p.get('num', i)}. <a href='tg://user?id={p['id']}'>{p['name']}</a>{cap}\n"
    return text
...
async def afk_warning_10(context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.get('mode') == 'SOLO':
        ...
        if role == 'BATTER' and (not game.get('striker') or game['striker']['id'] != user_id): return
        if role == 'BOWLER' and (not game.get('current_bowler') or game['current_bowler']['id'] != user_id): return
...
async def afk_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.get('mode') == 'SOLO':
        ...
        player = next((p for p in game['team_a']['players'] + game['team_b']['players'] if p['id'] == user_id), None)
        await context.bot.send_message(chat_id, f"⏳ <b>TIME'S UP!</b> {player['name']} was AFK for 90 seconds and has forfeited their turn! ❌", parse_mode='HTML')
        ...
        if role == 'BATTER':
            ...
            await context.bot.send_message(chat_id, f"🏏 {player['name']} timed out and is OUT! Captain/Host, please select the next batter using /batting [number].")
        elif role == 'BOWLER':
            game['waiting_for'] = 'TEAM_BOWLER_SELECT'
            await context.bot.send_message(chat_id, f"🥎 {player['name']} timed out! Captain/Host, please select a new bowler to continue the over using /bowling [number].")
...
async def team_join_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(game['team_a']['players']) < 2 or len(game['team_b']['players']) < 2:
        game['state'] = 'NOT_PLAYING'
        await context.bot.send_message(chat_id, "⏳ Time's up! Insufficient number of players in one or both teams (Min 2 per team). Match cancelled. 🛑")
        return
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                if game and game['state'] == 'PLAYING' and game.get('waiting_for') == 'BOWLER':
                    ...
                    if bowler and update.effective_user.id == bowler['id']:
                        ...
                        await update.message.reply_text(
                            "🥎 <b>Your Turn to Bowl!</b>\nType 0-6 or use your Special Delivery! 🤔👇", 
    ...
    if context.bot_data.get(chat_id, {}).get('state') not in ['NOT_PLAYING', None, 'TEAM_FINISHED']:
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(p['id'] == target_user.id for p in game['team_a']['players'] + game['team_b']['players']):
        await update.message.reply_text(f"❌ {target_user.first_name} is already in a team!")
        return
        
    new_player = {'id': target_user.id, 'name': target_user.first_name, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False}
    
    if game.get('state') != 'TEAM_JOINING':
        new_player['num'] = len(game[team_key]['players']) + 1
...
async def remove_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    removed = False
    for team_key in ['team_a', 'team_b']:
        for p in game[team_key]['players']:
    ...
    if removed:
        await update.message.reply_text(f"✅ <b>{target_user.first_name}</b> has been successfully removed from their team! 🚪", parse_mode='HTML')
    else:
        await update.message.reply_text(f"❌ {target_user.first_name} is not in any team!")
...
async def changecap_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != game.get('host_id'):
        await update.message.reply_text("⚠️ <b>Warning:</b> Only the Game Host can change the team captain!", parse_mode='HTML')
        return

    if not context.args:
        await update.message.reply_text("Usage: /changecap A OR /changecap B (while replying to a user or tagging @username)")
        return

    team_choice = context.args[0].upper()
    if team_choice not in ['A', 'B']:
        await update.message.reply_text("❌ Please specify team 'A' or 'B'. Example: /changecap A")
        return

    team_key = f"team_{team_choice.lower()}"
    target_user = None
    ...
    if not target_user:
        await update.message.reply_text("❌ Please reply to a user's message or tag them to assign as captain!")
        return

    if target_user.is_bot:
        await update.message.reply_text("❌ A bot cannot be the Captain!")
        return

    in_team = any(p['id'] == target_user.id for p in game[team_key]['players'])
    if not in_team:
        await update.message.reply_text(f"❌ {target_user.first_name} is not in Team {team_choice}!")
        return

    game[team_key]['captain'] = target_user.id
    await update.message.reply_text(f"👑 <b>{target_user.first_name}</b> is now the Captain of Team {team_choice}! ⚡", parse_mode='HTML')
...
async def matchscore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if update.effective_chat.type == 'private': return
    
    game = context.bot_data.get(chat_id)
    if not game or game.get('state') in ['NOT_PLAYING', None]:
        await update.message.reply_text("❌ No active match is currently being played!")
        return
        
    scorecard = generate_scorecard(game)
    await update.message.reply_photo(photo=SCOREBOARD_IMG, caption=scorecard, parse_mode='HTML')
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = context.bot_data.get(chat_id)
    if not game or game.get('mode') != 'TEAM' or game.get('state') != 'PLAYING': return
    ...
    if update.effective_user.id not in [batting_team['captain'], game['host_id']]: return
    ...
    if selected.get('is_out'):
        ...
        return
    if game.get('striker', {}).get('id') == selected['id'] or game.get('non_striker', {}).get('id') == selected['id']:
    ...
    if game['waiting_for'] == 'TEAM_OPENERS_BAT':
        if not game.get('striker'):
            ...
            await update.message.reply_text(f"🏏 <b>{selected['name']}</b> selected as Non-Striker!\n\nBowling Team Captain/Host, type /bowling to see bowlers or /bowling [num] to select opening bowler.", parse_mode='HTML')
            game['waiting_for'] = 'TEAM_BOWLER_SELECT'
    else:
        game['striker'] = selected
        selected['is_striker'] = True
        ...
        if game.get('need_new_bowler'):
            ...
            game['waiting_for'] = 'TEAM_BOWLER_SELECT'
            await update.message.reply_text("Bowling Captain/Host, please select the next bowler using /bowling [num].", parse_mode='HTML')
        else:
            game['waiting_for'] = 'BOWLER'
            await trigger_bowl(context, chat_id)
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        ...
        for p in bowling_team['players']:
            ...
            if game.get('current_bowler', {}).get('id') == p['id']:
                status = "🥎 (Bowling Now)"
    ...
    if update.effective_user.id not in [bowling_team['captain'], game['host_id']]: return
...
async def trigger_full_scorecard_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, game_data):
    ...
    final_caption = f"🏆 <b>MATCH SCORE!</b>\n{scorecard}{potm}"
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    dm_text += "👉 <b>Your Turn to Bowl!</b> Type a number from 0 to 6."
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        if game and game.get('state') not in ['NOT_PLAYING', None, 'TEAM_FINISHED']:
        ...
        await context.bot.send_photo(
            chat_id=chat_id,
            photo='https://res.cloudinary.com/dxgfxfoog/image/upload/v1777710602/file_000000005a607208bd015e237540e8e3_iplfhm.png',
        ...
        await context.bot.send_photo(
            chat_id=chat_id,
            photo='https://res.cloudinary.com/dxgfxfoog/image/upload/v1777710675/file_0000000017e07208be22ea00fbd507ad_dbvkje.png',
            caption=text,
            parse_mode='HTML',
        ...
        if not game or game.get('state') != 'TEAM_JOINING': return
        if any(p['id'] == user_id for p in game['team_a']['players'] + game['team_b']['players']):
            await context.bot.send_message(chat_id, f"❌ {update.effective_user.first_name}, you are already in a team!")
            return
        game['team_a']['players'].append({'id': user_id, 'name': update.effective_user.first_name, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False})
        ...
        if any(p['id'] == user_id for p in game['team_a']['players'] + game['team_b']['players']):
            await context.bot.send_message(chat_id, f"❌ {update.effective_user.first_name}, you are already in a team!")
            return
        game['team_b']['players'].append({'id': user_id, 'name': update.effective_user.first_name, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False})
        ...
        if not game or game.get('state') != 'TEAM_CAPTAINS': return
        ...
        if not game or game.get('state') != 'TEAM_TOSS': return
        ...
        if won_toss:
            ...
            await query.edit_message_text(f"🎉 You won the toss! What will you do?", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
        else:
            game['state'] = 'TEAM_TOSS_DECISION'
        ...
        await query.edit_message_text(f"✅ The captain chose to {dec_text} first!", reply_markup=InlineKeyboardMarkup([]))
        ...
        if not game or game.get('state') != 'TEAM_OVERS': return
        ...
        if action == 'yes':
            ...
            if game_ref: 
                ...
                for prefix in ['autostart_', 'team_join_', 'afk10_', 'afk60_', 'afk90_']:
        ...
        if roll <= 60: # 60% WIDE
            await query.edit_message_text("🚨 You bowled a <b>WIDE!</b> 1 extra run. You must bowl again.", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            batter['runs'] += 1
            bowler['conceded'] += 1
            ...
            await query.edit_message_text("🚨 <b>You bowled a NO BALL!</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            ...
            await context.bot.send_animation(
                ...
                caption=f"🚨 Ball bowled! 🥎💨\n👉 <a href='tg://user?id={batter['id']}'>{batter['name']}</a>, type 0-6 to hit! 🏏👇", 
                parse_mode='HTML'
            )
            set_afk_timer(context, group_id, batter['id'], 'BATTER')
            
        else: # 20% YORKER & BOWLED
            await query.edit_message_text("💥 <b>PERFECT YORKER!</b> You shattered the stumps!", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            ...
            if is_free_hit:
                ...
                await context.bot.send_animation(group_id, animation=MEDIA['batter_turn'], caption=result_text, parse_mode='HTML')
            else:
                bowler['wickets'] += 1
                result_text = f"🥎 Bowler delivery: <b>YORKER</b>\n🏏 Batter hit: <b>MISSED</b>\n\n"
                result_text += f"💥 <b>HOWZAT! OUT!</b> ☝️ {batter['name']} is dismissed for {batter['runs']}! 😔🚶‍♂️"
                
                await context.bot.send_animation(group_id, animation=MEDIA['yorker'], caption=result_text, parse_mode='HTML')
                
                if batter['runs'] == 0:
                    duck_text = f"🦆 <a href='tg://user?id={batter['id']}'>{batter['name']}</a> got a duck 🦆"
                    await context.bot.send_animation(group_id, animation=MEDIA['duck'], caption=duck_text, parse_mode='HTML')
                ...
                if game.get('mode') == 'TEAM':
                    game['batting_team_ref']['wickets'] += 1
                    if game['batting_team_ref']['wickets'] >= len(game['batting_team_ref']['players']) - 1:
                        await process_team_innings_end(context, group_id, game)
                        return
                    else:
                        game['waiting_for'] = 'TEAM_BATTER_SELECT'
                        await context.bot.send_message(group_id, f"🏏 Captain/Host, type /batting to view available batters or /batting [number] to select the next batter.")
                else:
                    game['batter_idx'] += 1
            ...
            if is_over_complete:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    spell_text += f"Bowling Captain/Host, select next bowler using /bowling [num]."
                    await context.bot.send_message(group_id, spell_text, parse_mode='HTML')
                    ...
                    spell_text += generate_scorecard(game)
                    await context.bot.send_message(group_id, spell_text, parse_mode='HTML')
                    game['balls_bowled'] = 0
                    game['special_used_this_over'] = False
                    game['bowler_idx'] = (game['bowler_idx'] + 1) % len(game['players'])
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_input.isdigit() or not (0 <= int(user_input) <= 6): return
    ...
    # GROUP INPUT
            ...
            if is_free_hit:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    if game['batting_team_ref']['wickets'] >= len(game['batting_team_ref']['players']) - 1:
                        ...
                        game['waiting_for'] = 'TEAM_BATTER_SELECT'
                        await context.bot.send_message(chat_id, f"🏏 Captain/Host, type /batting to see batters list or /batting [number] to select the next batter.")
                else:
                    game['batter_idx'] += 1
                    if game['batter_idx'] >= len(game['players']):
                        game['state'] = 'NOT_PLAYING'
        ...
        if is_over_complete:
            ...
            if game.get('mode') == 'TEAM':
                ...
                team = game['batting_team_ref']
                spell_text += f"\n📊 Score: {team['score']}/{team['wickets']}\n"
                spell_text += f"Bowling Captain/Host, select next bowler using /bowling to see list or /bowling [num]."
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler('changecap', changecap_command)) # Added changecap command
    ...
    app.add_handler(CommandHandler('matchscore', matchscore_command))
    app.add_handler(CommandHandler('score', matchscore_command)) # Added score command
