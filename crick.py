...
def generate_scorecard(game):
    ...
    for p in game['players']:
        text += f"👤 <b>{p['name']}</b>: {p['runs']} Runs | {p['wickets']} Wickets (Conceded: {p['conceded']})\n"
    return text
...
def get_potm(game):
    ...
    best_score = -999
    for p in game['players']:
        # Simple algorithm to determine POTM: 1 point per run, 15 per wicket, -0.5 per run conceded
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Added 'wickets' tracker
    game['players'].append({'id': user.id, 'name': user.first_name, 'runs': 0, 'conceded': 0, 'wickets': 0})
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        ...
        if roll <= 85: # WIDE
            ...
            bowler_ref['wickets'] += 1  # Award wicket
            ...
            if game['balls_bowled'] >= game['spell']:
                ...
                result_text += generate_scorecard(game) # Over scorecard
                game['balls_bowled'] = 0
                game['special_used_this_over'] = False
                game['bowler_idx'] = (game['bowler_idx'] + 1) % len(game['players'])
                if game['bowler_idx'] == game['batter_idx']:
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # GROUP INPUT
        ...
        # Thunder emoji delay before responding
        ...
        if hit_val == bowl_val and bowl_val != 'NO_BALL':
            if is_free_hit:
                ...
                bowler['wickets'] += 1 # Award wicket to bowler
                result_text = f"🥎 Bowler delivery: <b>{bowl_val}</b>\n🏏 Batter hit: <b>{hit_val}</b>\n\n"
                if batter['runs'] == 0:
                    media_url = MEDIA['duck']
                    result_text += f"🦆 <b>GOLDEN DUCK!</b> {batter['name']} is OUT for 0! 😭🚶‍♂️"
