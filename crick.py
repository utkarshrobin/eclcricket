...
MEDIA = {
    ...
    'bowler_turn': 'https://tenor.com/view/gullybet-wicket-out-cricket-crickets-stamp-out-gif-6360097491920935496',
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    # Safeguard against Tenor links crashing the send_animation API
    try:
        if "tenor.com/view" in MEDIA['bowler_turn']:
            raise ValueError("Tenor link detected")
        ...
        # Fallback: Sends the Tenor link inside text, which Telegram instantly converts to a playing GIF preview
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"<a href='{MEDIA['bowler_turn']}'>&#8205;</a>" + group_text,
            reply_markup=InlineKeyboardMarkup(group_kb),
            parse_mode='HTML'
        )
