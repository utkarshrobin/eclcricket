...
def generate_teams_message(game):
    ...
    for i, p in enumerate(game['team_a']['players'], 1):
        ...
        text += f" {i}. <a href='tg://user?id={p['id']}'>{p['name']}</a>{cap}\n"
...
async def afk_warning_10(context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.get('mode') == 'SOLO':
        ...
        if role == 'BATTER' and game['striker']['id'] != user_id: return
        if role == 'BOWLER' and game['current_bowler']['id'] != user_id: return
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data.get(chat_id, {}).get('state') not in ['NOT_PLAYING', None]:
...
async def create_team_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚔️ <b>TEAM REGISTRATION OPEN!</b> ⚔️\n\nPlayers, choose your sides! You have 60 seconds to join. ⏳\n*(Host can type /rejoin to extend 30s or /add a|b to manually add players)*", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != game.get('host_id') and not await is_admin(update.effective_chat, update.effective_user.id):
        await update.message.reply_text("❌ Only the Game Host or an Admin can add players manually!")
        return
        ...
        try:
            ...
            for entity in update.message.entities:
                if entity.type == 'text_mention':
                    ...
                    break
            if not target_user:
                await update.message.reply_text(f"❌ Could not find user {username}. Try replying to their message instead!")
                return
    ...
    # If added after joining phase (e.g. late replacement), assign a number right away
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: /batting [player_number]")
        return
    ...
    if game['waiting_for'] == 'TEAM_OPENERS_BAT':
        if not game.get('striker'):
            ...
            await update.message.reply_text(f"🏏 <b>{selected['name']}</b> selected as Non-Striker!\n\nBowling Team Captain/Host, type /bowling [num] to select the opening bowler.", parse_mode='HTML')
            game['waiting_for'] = 'TEAM_BOWLER_SELECT'
    else:
        game['striker'] = selected
        selected['is_striker'] = True
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("Usage: /bowling [player_number]")
        return
        ...
        if roll <= 85: # WIDE
            ...
            if is_free_hit:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    if game['batting_team_ref']['wickets'] >= len(game['batting_team_ref']['players']) - 1:
                        ...
                        game['waiting_for'] = 'TEAM_BATTER_SELECT'
                        await context.bot.send_message(group_id, f"🏏 Captain/Host, type /batting [number] to select the next batter.")
                else:
                    game['batter_idx'] += 1
            ...
            if is_over_complete:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    spell_text += f"\nBowling Captain/Host, select next bowler using /bowling [num]."
            ...
            await query.edit_message_text("Choice locked! 🔒 You bowled a <b>NO BALL</b>.", parse_mode='HTML')
    ...
    # GROUP INPUT
            ...
            if is_free_hit:
                ...
                if game.get('mode') == 'TEAM':
                    ...
                    if game['batting_team_ref']['wickets'] >= len(game['batting_team_ref']['players']) - 1:
                        ...
                        game['waiting_for'] = 'TEAM_BATTER_SELECT'
                        await context.bot.send_message(chat_id, f"🏏 Captain/Host, type /batting [number] to select the next batter.")
                else:
                    game['batter_idx'] += 1
                    if game['batter_idx'] >= len(game['players']):
                        game['state'] = 'NOT_PLAYING'
        ...
        if is_over_complete:
            ...
            if game.get('mode') == 'TEAM':
                ...
                spell_text += f"Batting Captain/Host, select next bowler using /bowling [num]."
