...
def generate_team_scorecard(game):
    ...
    for team_key, team_name in [('team_a', '🔴 Team A'), ('team_b', '🔵 Team B')]:
        ...
        played_overs = opp_team['balls_bowled'] // 6 if opp_team else 0
        played_balls = opp_team['balls_bowled'] % 6 if opp_team else 0
            
        text += f"🏏 <b>{team_name}</b> ➯ <b>{team['score']}/{team['wickets']}</b> <i>({played_overs}.{played_balls} Ov)</i>\n\n"
        
        text += "<b>Top Batters:</b>\n"
        batters = sorted(team['players'], key=lambda x: x['runs'], reverse=True)[:3]
        for p in batters:
            if p['balls_faced'] > 0 or p.get('is_striker') or p.get('is_non_striker'):
                status = "🏏" if not p['is_out'] else "❌"
                sr = (p['runs'] / p['balls_faced'] * 100) if p['balls_faced'] > 0 else 0
                text += f"  {status} {p['name'][:12]} ➯ {p['runs']} ({p['balls_faced']}) <i>SR: {sr:.0f}</i>\n"
                
        text += "\n<b>Top Bowlers:</b>\n"
        bowlers = sorted(team['players'], key=lambda x: x['wickets'], reverse=True)[:3]
        for p in bowlers:
            if p['balls_bowled'] > 0:
                p_ov = p['balls_bowled'] // 6
                p_bl = p['balls_bowled'] % 6
                eco = (p['conceded'] / p['balls_bowled']) * 6
                text += f"  🥎 {p['name'][:12]} ➯ {p['wickets']}-{p['conceded']} <i>({p_ov}.{p_bl} Ov) Eco: {eco:.1f}</i>\n"
        text += "═" * 25 + "\n"
        
    if game['state'] == 'TEAM_FINISHED':
        text += "\n🏆 <b>MATCH RESULT</b> 🏆\n"
        
        team2 = game['batting_team_ref'] 
        team1 = game['bowling_team_ref']
        
        if team2['score'] >= game['target']:
            wkts_left = max(0, len(team2['players']) - team2['wickets'] - 1)
            rem_balls = (game['target_overs'] * 6) - team1['balls_bowled']
            rem_ov = rem_balls // 6
            rem_bl = rem_balls % 6
            t2_name = "Team A" if team2 == game['team_a'] else "Team B"
            text += f"🎉 <b>{t2_name} Wins by {wkts_left} wickets!</b> ({rem_ov}.{rem_bl} overs left) 🎉\n"
        elif team1['score'] > team2['score']:
            diff = team1['score'] - team2['score']
            t1_name = "Team A" if team1 == game['team_a'] else "Team B"
            text += f"🎉 <b>{t1_name} Wins by {diff} runs!</b> 🎉\n"
        else:
            text += "🤝 <b>IT\'S A THRILLING TIE!</b> 🤝\n"
...
def set_afk_timer(context, chat_id, user_id, role):
    ...
    if game.get('mode') == 'TEAM':
        ...
        # Solo Mode: 60s total timer
        context.job_queue.run_once(afk_warning_start, 0, data={'chat_id': chat_id, 'user_id': user_id, 'role': role}, name=f"afk0_{chat_id}")
...
def clear_afk_timer(context, chat_id):
    for prefix in ['afk0_', 'afk10_', 'afk30_', 'afk60_', 'afk90_']:
...
async def afk_warning_start(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(chat_id, f"⚠️ <a href='tg://user?id={user_id}'>{player['name']}</a>, your turn! You have <b>60 seconds</b> to play. ⏳", parse_mode='HTML')
...
async def queue_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('state') != 'JOINING' or game.get('mode') != 'SOLO':
        ...
        return
    try:
        await context.bot.send_message(chat_id, f"⏳ <b>Queue is open!</b> Type /join to enter the match! (Total: {len(game['players'])}) 🏏", parse_mode='HTML')
    except Exception: pass
...
async def trigger_team_captains(context, chat_id, game):
    ...
    try:
        ...
        await context.bot.send_message(chat_id, text=roster, parse_mode='HTML')
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == 'private':
            ...
            try:
                ...
                if game and game['state'] == 'PLAYING' and game.get('waiting_for') == 'BOWLER':
                    ...
                    if bowler and update.effective_user.id == bowler['id']:
                        ...
                        await message.reply_text("It is not your turn to bowl right now! 🚫🏏")
                        return
            except ValueError:
                pass
                
        welcome_private = (
            "<b>Welcome to the ELITE CRICKET BOT! 🏏</b>\n\n"
            "Step onto the virtual pitch and experience the thrill of Telegram's most dynamic cricket simulator. Whether you're a casual fan or a hardcore strategist, our bot brings the excitement of T20 right to your fingertips.\n\n"
            "Challenge your friends in intense Team Modes, test your individual skills in Solo matches, and track your stats as you climb the leaderboards. With real-time bowling, batting choices, and dynamic match conditions, every game is a new adventure!\n\n"
            "Ready to hit a six? <b>Join our official community at @eclplays</b> to participate in tournaments, find opponents, and stay updated on the latest features!"
        )
        kb_private = [
        ...
        try:
            await message.reply_photo(
                photo='https://res.cloudinary.com/dxgfxfoog/image/upload/v1777784450/1777784348551-01_brdteb.jpg',
                caption=welcome_private,
                reply_markup=InlineKeyboardMarkup(kb_private),
                parse_mode='HTML'
            )
        except Exception:
            await message.reply_text(welcome_private, reply_markup=InlineKeyboardMarkup(kb_private), parse_mode='HTML')
        return

    # Group Start logic
    current_time = time.time()
    last_time = context.bot_data.get(f'last_start_{chat_id}', 0)
    if current_time - last_time < 7:
        remaining = int(7 - (current_time - last_time))
        # Ensure we don't spam the cooldown message 
        if current_time - context.bot_data.get(f'last_cooldown_msg_{chat_id}', 0) > 7:
            try:
                await message.reply_text(f"⏳ The /start command is under cooldown. Please try again after {remaining} seconds.")
                context.bot_data[f'last_cooldown_msg_{chat_id}'] = current_time
            except Exception: pass
        return
    context.bot_data[f'last_start_{chat_id}'] = current_time
    ...
    if game.get('state') not in ['NOT_PLAYING', None, 'TEAM_FINISHED']:
        try:
            await message.reply_text("❌ A match is already active in this group! Finish it or ask an admin to /endmatch first.")
        except Exception: pass
        return
    ...
    try:
        ...
        # Fallback if image fetching fails due to Cloudinary or Telegram API errors
        await message.reply_text(
            text=welcome_text,
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='HTML'
...
async def create_team_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != game.get('host_id'):
        await update.effective_message.reply_text("❌ Only the Game Host can create the teams!")
        return
    ...
    context.job_queue.run_once(team_join_timeout, 60, data={'chat_id': chat_id}, name=f"team_join_{chat_id}")
    await update.effective_message.reply_text("⚔️ <b>TEAM REGISTRATION OPEN!</b> ⚔️\n\nPlayers, choose your sides! You have 60 seconds to join. ⏳\n<b>(Host can type /rejoin to extend 30s or use /add or /remove)</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
...
async def rejoin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("⏳ <b>Registration Extended!</b> 30 more seconds to join the teams! 👥", parse_mode='HTML')
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('mode') != 'TEAM': 
        await update.effective_message.reply_text("❌ No active team match setup found!")
        return
        
    if update.effective_user.id != game.get('host_id'):
        await update.effective_message.reply_text("❌ Only the Game Host can add players manually!")
        return
        
    if not context.args:
        await update.effective_message.reply_text("Usage: /add a OR /add b (while replying to a user's message or tagging @username)")
        return
        
    team_choice = context.args[0].lower()
    if team_choice not in ['a', 'b']:
        await update.effective_message.reply_text("❌ Please specify team 'a' or 'b'. Example: /add a")
        return
    ...
    if not target_user:
        await update.effective_message.reply_text("❌ Please reply to a user's message or tag their @username properly!")
        return
        
    if target_user.is_bot:
        await update.effective_message.reply_text("❌ You cannot add a bot to the team!")
        return
        
    if is_user_playing_anywhere(context, target_user.id):
        await update.effective_message.reply_text(f"❌ {target_user.first_name} is already in a match or queue in another group! They can only play one game at a time.")
        return
        
    in_a = any(p['id'] == target_user.id for p in game['team_a']['players'])
    in_b = any(p['id'] == target_user.id for p in game['team_b']['players'])
    if in_a:
        await update.effective_message.reply_text(f"❌ {target_user.first_name} is already in Team A 🔴!")
        return
    if in_b:
        await update.effective_message.reply_text(f"❌ {target_user.first_name} is already in Team B 🔵!")
        return
        
    username = target_user.username.lower() if target_user.username else None
    new_player = {'id': target_user.id, 'name': target_user.first_name, 'username': username, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False}
    ...
    await update.effective_message.reply_text(f"✅ <b>{target_user.first_name}</b> has been manually added to {team_name} by the Host! 👥", parse_mode='HTML')
...
async def remove_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != game.get('host_id'):
        await update.effective_message.reply_text("❌ Only the Game Host can remove players manually!")
        return
    ...
    if message.reply_to_message:
        ...
        for entity in message.entities:
            if entity.type == 'text_mention':
                ...
                target_username = message.text[entity.offset:entity.offset+entity.length].lstrip('@').lower()
                break
    ...
    if removed:
        await update.effective_message.reply_text(f"✅ <b>{target_name}</b> has been successfully removed from their team! Numbers updated. 🚪", parse_mode='HTML')
    else:
        name = target_user.first_name if target_user else target_username
        await update.effective_message.reply_text(f"❌ {name} is not in any team!")
...
async def changehost_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('mode') != 'TEAM' or game.get('state') in ['NOT_PLAYING', None, 'TEAM_FINISHED']: 
        await update.effective_message.reply_text("❌ No active team match to change host!")
        return
    ...
    if not target_user:
        await update.effective_message.reply_text("❌ Please reply to a user's message or tag their @username!")
        return
        
    if target_user.is_bot:
        await update.effective_message.reply_text("❌ A bot cannot be the Game Host!")
        return
        
    if update.effective_user.id == game.get('host_id'):
        game['host_id'] = target_user.id
        await update.effective_message.reply_text(f"✅ Host privileges successfully transferred to <b>{target_user.first_name}</b>! 👑", parse_mode='HTML')
    else:
        ...
        await update.effective_message.reply_text(f"🗳️ Vote initiated to change host to <b>{target_user.first_name}</b>!\n4 votes required.", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('state') != 'JOINING':
        await update.effective_message.reply_text("No match is open for joining! Type /start ❌🏏")
        return

    user = update.effective_user

    if is_user_playing_anywhere(context, user.id):
        await update.effective_message.reply_text(f"❌ {user.first_name}, you are already in a match or queue in another group! You can only play one game at a time.")
        return

    if any(p['id'] == user.id for p in game['players']):
        await update.effective_message.reply_text(f"⚠️ Hey {user.first_name}, you are ALREADY in the queue! 🧍‍♂️⏳ Just wait for the match to start! 🏏🔥")
        return

    username = user.username.lower() if user.username else None
    game['players'].append({'id': user.id, 'name': user.first_name, 'username': username, 'runs': 0, 'conceded': 0, 'wickets': 0, 'balls_bowled': 0, 'balls_faced': 0})
    
    timer_msg = ""
    if len(game['players']) == 1:
        context.job_queue.run_once(auto_start_match, 70, data={'chat_id': chat_id}, name=f"autostart_{chat_id}")
        context.job_queue.run_repeating(queue_reminder, interval=10, first=10, data={'chat_id': chat_id}, name=f"queueremind_{chat_id}")
        timer_msg = "\n⏳ <i>Auto-start timer initiated: Match begins in 70 seconds!</i>"
        
    await update.effective_message.reply_text(f"✅ <b>{user.first_name}</b> joined! (Total: {len(game['players'])}) 👥{timer_msg}", parse_mode='HTML')
...
async def leavesolo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.get('state') == 'PLAYING':
        await update.effective_message.reply_text("❌ The match has already started! You can't leave now.")
        return
    ...
    if game.get('state') == 'JOINING':
        ...
        if player:
            game['players'] = [p for p in game['players'] if p['id'] != user_id]
            await update.effective_message.reply_text(f"👋 <b>{update.effective_user.first_name}</b> has left the queue. (Total: {len(game['players'])}) 👥", parse_mode='HTML')
            
            if len(game['players']) == 0:
                for prefix in ['autostart_', 'queueremind_']:
                    jobs = context.job_queue.get_jobs_by_name(f"{prefix}{chat_id}")
                    for job in jobs: job.schedule_removal()
                await update.effective_message.reply_text("Queue is empty! 🏏 Timer stopped.")
        else:
            await update.effective_message.reply_text("You are not in the queue! ❌")
...
async def startsolo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update.effective_chat, update.effective_user.id):
        await update.effective_message.reply_text("❌ Only group admins can start the match manually!")
        return

    game = context.bot_data.get(chat_id)
    if not game or game['state'] != 'JOINING': return
    if len(game['players']) < 2:
        await update.effective_message.reply_text("We need at least 2 players! 👥🏏")
        return
    ...
    await update.effective_message.reply_text("🚨 <b>THE MATCH HAS BEGUN!</b> 🚨", parse_mode='HTML')
    await trigger_bowl(context, chat_id)
...
async def endmatch_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update.effective_chat, update.effective_user.id):
        await update.effective_message.reply_text("❌ Only group admins can end the match!")
        return
        
    game = context.bot_data.get(chat_id)
    if not game or game.get('state') in ['NOT_PLAYING', None, 'TEAM_FINISHED']:
        await update.effective_message.reply_text("❌ There is no active match to end!")
        return
    ...
    await update.effective_message.reply_text("⚠️ <b>Admin Action:</b> Are you sure you want to force-end the current match?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
...
async def soloscore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('mode') != 'SOLO' or game.get('state') in ['NOT_PLAYING', None]:
        await update.effective_message.reply_text("❌ No active solo match is currently being played!")
        return
...
async def teamscore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('mode') != 'TEAM' or game.get('state') in ['NOT_PLAYING', None]:
        await update.effective_message.reply_text("❌ No active team match is currently being played!")
        return
...
async def teams_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get('mode') != 'TEAM' or game.get('state') in ['NOT_PLAYING', 'TEAM_SETUP_HOST', 'TEAM_JOINING']:
        await update.effective_message.reply_text("❌ No active team match right now!")
        return
        
    roster = generate_teams_message(game)
    try:
        await update.effective_message.reply_photo(photo=TEAMS_ROSTER_IMG, caption=roster, parse_mode='HTML')
    except Exception:
        await update.effective_message.reply_text(text=roster, parse_mode='HTML')
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = context.bot_data.get(chat_id)
    if not game or game.get('mode') != 'TEAM': 
        await update.effective_message.reply_text("❌ There is no active team match currently! This command is only for team matches.")
        return
    if game.get('state') != 'PLAYING': 
        await update.effective_message.reply_text("❌ The match hasn't started yet!")
        return
    ...
    if update.effective_user.id not in [batting_team['captain'], game['host_id']]:
        await update.effective_message.reply_text("❌ Only the Game Host or Batting Captain can select the batter!")
        return
    ...
    if not selected:
        await update.effective_message.reply_text(f"❌ Player {p_num} not found in your team!")
        return
    if selected.get('is_out'):
        await update.effective_message.reply_text(f"❌ {selected['name']} is already out!")
        return
    
    st = game.get('striker') or {}
    ns = game.get('non_striker') or {}
    if st.get('id') == selected['id'] or ns.get('id') == selected['id']:
        await update.effective_message.reply_text(f"❌ {selected['name']} is already on the pitch!")
        return
    ...
    if game['waiting_for'] == 'TEAM_OPENERS_BAT':
        if not game.get('striker'):
            ...
            await update.effective_message.reply_text(f"🏏 <b>{selected['name']}</b> selected as Striker!", parse_mode='HTML')
        elif not game.get('non_striker'):
            game['non_striker'] = selected
            selected['is_non_striker'] = True
            
            openers_gif = "https://giphy.com/gifs/team-india-cricket-lover-handshake-roko-hGJTJqTNaj0XXkLXZr"
            ...
            try:
                await context.bot.send_animation(chat_id, animation=openers_gif, caption=caption_txt, parse_mode='HTML')
            except:
                await context.bot.send_message(chat_id, caption_txt, parse_mode='HTML')
                
            game['waiting_for'] = 'TEAM_BOWLER_SELECT'
    else:
        game['striker'] = selected
        selected['is_striker'] = True
        await update.effective_message.reply_text(f"🏏 <b>{selected['name']}</b> walks out to the pitch!", parse_mode='HTML')
        
        if game.get('need_new_bowler'):
            game['need_new_bowler'] = False
            game['waiting_for'] = 'TEAM_BOWLER_SELECT'
            await update.effective_message.reply_text("Bowling Captain/Host, please select the next bowler using <code>/bowling [num]</code>.", parse_mode='HTML')
        else:
            game['waiting_for'] = 'BOWLER'
            await trigger_bowl(context, chat_id)
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id not in [bowling_team['captain'], game['host_id']]:
        await update.effective_message.reply_text("❌ Only the Game Host or Bowling Captain can select the bowler!")
        return
    ...
    if game.get('last_bowler_id') == selected['id']:
        await update.effective_message.reply_text("❌ A bowler cannot bowl two consecutive overs!")
        return
        
    game['current_bowler'] = selected
    game['waiting_for'] = 'BOWLER'
    
    await update.effective_message.reply_text(f"🥎 <b>{selected['name']}</b> is handed the ball!", parse_mode='HTML')
    
    if game.get('innings_start_msg_pending'):
        game['innings_start_msg_pending'] = False
        await update.effective_message.reply_text("🚨 <b>THE INNINGS HAS BEGUN!</b> 🚨", parse_mode='HTML')
...
async def trigger_full_scorecard_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, game_data):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text=final_caption, parse_mode='HTML')
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    try:
        if "tenor.com/view" in MEDIA['bowler_turn']:
            raise ValueError("Tenor link")
        await context.bot.send_animation(chat_id=chat_id, animation=MEDIA['bowler_turn'], caption=group_text, reply_markup=InlineKeyboardMarkup(group_kb), parse_mode='HTML')
    except Exception:
        await context.bot.send_message(chat_id=chat_id, text=f"<a href='{MEDIA['bowler_turn']}'>&#8205;</a>" + group_text, reply_markup=InlineKeyboardMarkup(group_kb), parse_mode='HTML')
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        if game.get('state') not in ['NOT_PLAYING', None, 'TEAM_FINISHED']:
            await query.edit_message_caption(caption="❌ A match is already active or setting up in this group!", reply_markup=InlineKeyboardMarkup([]))
            return
        ...
        try:
            ...
            await context.bot.send_message(chat_id, "Select Spell Limit: ⚖️🏏", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        if is_user_playing_anywhere(context, user_id):
            await query.answer("❌ You are already playing/hosting in another group! You can only be in one game.", show_alert=True)
            return
        context.bot_data[chat_id] = {'state': 'TEAM_SETUP_HOST', 'host_id': user_id, 'mode': 'TEAM'}
        try:
            await query.edit_message_caption(caption=f"👑 <a href='tg://user?id={user_id}'>{update.effective_user.first_name}</a> is the Game Host!\n\nHost, please send /create_team to open the team registration.", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
        except Exception:
            await query.edit_message_text(text=f"👑 <a href='tg://user?id={user_id}'>{update.effective_user.first_name}</a> is the Game Host!\n\nHost, please send /create_team to open the team registration.", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))

    elif query.data == 'join_team_a':
        if game.get('state') != 'TEAM_JOINING': return
        if is_user_playing_anywhere(context, user_id):
            await context.bot.send_message(chat_id, f"❌ {update.effective_user.first_name}, you are already in a match or queue in another group! You can only play one game at a time.")
            return
        if any(p['id'] == user_id for p in game['team_a']['players']):
            await query.answer("⚠️ You are already in Team A! 🔴", show_alert=True)
            return
        if any(p['id'] == user_id for p in game['team_b']['players']):
            await query.answer("⚠️ You are already in Team B! 🔵", show_alert=True)
            return
            
        username = update.effective_user.username.lower() if update.effective_user.username else None
        game['team_a']['players'].append({'id': user_id, 'name': update.effective_user.first_name, 'username': username, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False})
        ...
        username = update.effective_user.username.lower() if update.effective_user.username else None
        game['team_b']['players'].append({'id': user_id, 'name': update.effective_user.first_name, 'username': username, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False})
        ...
        if game['team_a']['captain'] and game['team_b']['captain']:
            ...
            toss_gif = "https://giphy.com/gifs/cricket-wicket-fancode-3Jt57qFHMiCeHMuJ6O"
            
            try:
                await context.bot.send_animation(chat_id, animation=toss_gif, caption=f"🪙 <b>TOSS TIME!</b>\n<a href='tg://user?id={cap_id}'>{cap_name}</a>, call the toss!", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
            except Exception:
                await context.bot.send_message(chat_id, f"🪙 <b>TOSS TIME!</b>\n<a href='tg://user?id={cap_id}'>{cap_name}</a>, call the toss!", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
        ...
        if won_toss:
            ...
            try:
                await query.edit_message_caption(caption=f"🎉 You won the toss! What will you do?", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
            except:
                await query.edit_message_text(text=f"🎉 You won the toss! What will you do?", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
        else:
            game['state'] = 'TEAM_TOSS_DECISION'
            ...
            kb = [[InlineKeyboardButton("Bat 🏏", callback_data="toss_bat"), InlineKeyboardButton("Bowl 🥎", callback_data="toss_bowl")]]
            try:
                await query.edit_message_caption(caption=f"❌ You lost the toss!\n\n<a href='tg://user?id={cap_id}'>{cap_name}</a> won the toss. What will you choose?", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
            except:
                await query.edit_message_text(text=f"❌ You lost the toss!\n\n<a href='tg://user?id={cap_id}'>{cap_name}</a> won the toss. What will you choose?", reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
        ...
        try:
            await query.edit_message_caption(caption=f"✅ The captain chose to {dec_text} first!", reply_markup=InlineKeyboardMarkup([]))
        except:
            await query.edit_message_text(text=f"✅ The captain chose to {dec_text} first!", reply_markup=InlineKeyboardMarkup([]))
        ...
        await query.edit_message_text(f"✅ Match set for <b>{overs} Overs</b> per side!", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
        ...
        if game.get('state') == 'PLAYING':
            try:
                await query.edit_message_caption(caption="❌ Match is already playing! Use /endmatch to stop it.", reply_markup=InlineKeyboardMarkup([]))
            except: pass
            return
        game['state'] = 'NOT_PLAYING'
        for prefix in ['autostart_', 'team_join_', 'queueremind_']:
            ...
            for job in jobs: job.schedule_removal()
        try:
            await query.edit_message_caption(caption="Setup cancelled. 🏏❌", reply_markup=InlineKeyboardMarkup([]))
        except: pass
        ...
        if votes >= 4:
            ...
            await query.edit_message_text(f"✅ Vote passed! Game Host successfully changed to <b>{game['host_vote_name']}</b>! 👑", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
        else:
        ...
        if action == 'yes':
            ...
            if game_ref: 
                ...
                for prefix in ['autostart_', 'team_join_', 'queueremind_', 'afk0_', 'afk10_', 'afk30_', 'afk60_', 'afk90_']:
                    jobs = context.job_queue.get_jobs_by_name(f"{prefix}{targ_chat_id}")
                    for job in jobs: job.schedule_removal()
            await query.edit_message_text("🛑 <b>Match has been force-ended by an Admin.</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
        elif action == 'no':
            await query.edit_message_text("✅ Force-end cancelled. The match continues!", reply_markup=InlineKeyboardMarkup([]))
        ...
        if roll <= 60: # 60% WIDE
            await query.edit_message_text("Oops! Missed yorker and gave a <b>WIDE</b> ball! 1 extra run. You must bowl again.", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            batter['runs'] += 1
            bowler['conceded'] += 1
        ...
        elif roll <= 80: # 20% NO BALL
            await query.edit_message_text("Oops! Missed yorker and gave a <b>NO BALL!</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == 'private':
        ...
        hit_opts = "1-6" if game.get('mode') == 'SOLO' else "0-6"
        await update.message.reply_text(f"Choice locked! 🔒 You bowled a <b>{val}</b>.", parse_mode='HTML')
        await context.bot.send_animation(group_id, animation=MEDIA['batter_turn'], caption=f"🚨 Ball bowled! 🥎💨\n👉 <a href='tg://user?id={batter['id']}'>{batter['name']}</a>, type {hit_opts} to hit! 🏏👇", parse_mode='HTML')
        set_afk_timer(context, group_id, batter['id'], 'BATTER')
    ...
    # GROUP INPUT (BATTER)
        ...
        if bowl_val == 'NO_BALL':
            ...
            if game.get('mode') == 'TEAM':
                survives = hit_val in [0, 1, 2, 3]
            else:
                survives = hit_val in [1, 2, 3]
            ...
            if not survives:
                if is_free_hit:
                    ...
                    await update.message.reply_animation(animation=MEDIA['batter_turn'], caption=result_text, parse_mode='HTML')
                else:
                    bowler['wickets'] += 1
                    result_text = f"🥎 Bowler delivery: <b>YORKER</b>\n🏏 Batter hit: <b>{hit_val}</b>\n\n"
                    result_text += f"💥 <b>HOWZAT! OUT!</b> ☝️ {batter['name']} is bowled by a lethal yorker for {batter['runs']}! 😔🚶‍♂️"
                    
                    await update.message.reply_animation(animation=MEDIA['yorker'], caption=result_text, parse_mode='HTML')
                    
                    if batter['runs'] == 0:
                        duck_text = f"🦆 <a href='tg://user?id={batter['id']}'>{batter['name']}</a> got a duck 🦆"
                        await update.message.reply_animation(animation=MEDIA['duck'], caption=duck_text, parse_mode='HTML')
            ...
            if is_free_hit:
                ...
                await update.message.reply_animation(animation=MEDIA['out'], caption=result_text, parse_mode='HTML')
