...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        if game.get('state') not in ['NOT_PLAYING', None, 'TEAM_FINISHED']:
            try: await query.edit_message_caption(caption="❌ A match is already active or setting up in this group!", reply_markup=InlineKeyboardMarkup([]))
            except: pass
            return
        ...
        if game['team_a']['captain'] and game['team_b']['captain']:
            ...
            try:
                await context.bot.send_animation(chat_id, animation=toss_vid, caption=caption_msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
            except Exception:
        ...
        try: await query.edit_message_text(f"✅ Match set for <b>{overs} Overs</b> per side!", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
        except: pass
        ...
        if game.get('state') == 'PLAYING':
            try: await query.edit_message_caption(caption="❌ Match is already playing! Use /endmatch to stop it.", reply_markup=InlineKeyboardMarkup([]))
            except: pass
            return
        game['state'] = 'NOT_PLAYING'
        for prefix in ['autostart_', 'team_join_', 'queueremind_']:
            jobs = context.job_queue.get_jobs_by_name(f"{prefix}{chat_id}")
            for job in jobs: job.schedule_removal()
        try: await query.edit_message_caption(caption="Setup cancelled. 🏏❌", reply_markup=InlineKeyboardMarkup([]))
        except: pass
        ...
        if votes >= 4:
            ...
            try: await query.edit_message_text(f"✅ Vote passed! Game Host successfully changed to <b>{game['host_vote_name']}</b>! 👑", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            except: pass
        else:
        ...
        if action == 'yes':
            ...
            if game_ref: 
                ...
                for prefix in ['autostart_', 'team_join_', 'queueremind_', 'afk1_', 'afk10_', 'afk30_', 'afk60_', 'afk90_']:
                    ...
                    for job in jobs: job.schedule_removal()
            try: await query.edit_message_text("🛑 <b>Match has been force-ended by an Admin.</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            except: pass
        elif action == 'no':
            try: await query.edit_message_text("✅ Force-end cancelled. The match continues!", reply_markup=InlineKeyboardMarkup([]))
            except: pass
        ...
        if roll <= 60:
            try: await query.edit_message_text("Oops! Missed yorker and gave a <b>WIDE</b> ball! 1 extra run. You must bowl again.", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            except: pass
            batter['runs'] += 1
            bowler['conceded'] += 1
            ...
            try: await query.edit_message_text("Oops! Missed yorker and gave a <b>NO BALL!</b>", parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            except: pass
            ...
            try: await query.edit_message_text(msg, parse_mode='HTML', reply_markup=InlineKeyboardMarkup([]))
            except: pass
    ...
    # GROUP INPUT (BATTER)
        ...
        if bowl_val == 'NO_BALL':
            ...
            if not survives:
                if is_free_hit:
                    ...
                    if bowler['consecutive_wickets'] == 3:
                        ...
                        try: await context.bot.send_animation(chat_id, animation=ht_vid, caption=ht_cap, parse_mode='HTML')
                        except Exception: await context.bot.send_message(chat_id, text=ht_cap, parse_mode='HTML')
                ...
                if old_runs < 100 and batter['runs'] >= 100:
                    milestone_media = MEDIA['100']
                    milestone_text = f"👑 <b>CENTURY! TAKE A BOW!</b> 💯🔥\n<a href='tg://user?id={batter['id']}'>{batter['name']}</a> has smashed a glorious century! What an innings! 🏏🏟️"
                    try: await context.bot.send_animation(chat_id, animation=milestone_media, caption=milestone_text, parse_mode='HTML')
                    except Exception: await context.bot.send_message(chat_id, milestone_text, parse_mode='HTML')
                elif old_runs < 50 and batter['runs'] >= 50:
                    milestone_media = MEDIA['50']
                    milestone_text = f"🏏 <b>HALF-CENTURY! BRILLIANT INNINGS!</b> 💥🙌\n<a href='tg://user?id={batter['id']}'>{batter['name']}</a> reaches 50! Keep it going! 🔥"
                    try: await context.bot.send_animation(chat_id, animation=milestone_media, caption=milestone_text, parse_mode='HTML')
            ...
            if is_free_hit:
                ...
                if batter['runs'] == 0:
                    duck_text = f"🦆 <a href='tg://user?id={batter['id']}'>{batter['name']}</a> got a duck 🦆"
                    try: await update.message.reply_animation(animation=MEDIA['duck'], caption=duck_text, parse_mode='HTML')
                    except Exception: await update.message.reply_text(duck_text, parse_mode='HTML')
