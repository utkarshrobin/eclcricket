...
def generate_team_scorecard(game):
    ...
    if game['state'] == 'TEAM_FINISHED':
        ...
        # Determine who batted first to calculate accurate chase conditions
        team2 = game['batting_team_ref'] # Team currently batting or just finished
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_time - last_time < 7:
        await update.message.reply_text(f"⏳ The /start command is under cooldown. Please try again after {int(7 - (current_time - last_time))} seconds.")
        return
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(p['id'] == user.id for p in game['players']):
        await update.message.reply_text(f"⚠️ Hey {user.first_name}, you are ALREADY in the queue! 🧍‍♂️⏳ Just wait for the match to start! 🏏🔥")
        return
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id not in [batting_team['captain'], game['host_id']]:
        await update.message.reply_text("❌ Only the Game Host or Batting Captain can select the batter!")
        return
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id not in [bowling_team['captain'], game['host_id']]:
        await update.message.reply_text("❌ Only the Game Host or Bowling Captain can select the bowler!")
        return
