...
def generate_team_scorecard(game):
    ...
    for team_key, team_name in [("team_a", "🔴 TEAM A"), ("team_b", "🔵 TEAM B")]:
        ...
        played_overs, rem_b = divmod(played_balls, 6)
        total_ov_played = played_overs + (rem_b / 6)
        rr = (team["score"] / total_ov_played) if total_ov_played > 0 else 0.0

        text += f"🎖️ <b>{team_name}</b>  {team['score']}/{team['wickets']}  ({played_overs}.{rem_b} Ov)  RR: {rr:.2f}\n"

        # Batters
        bat_lines = ""
        for p in team.get("players", []):
            if p.get("balls_faced", 0) > 0 or p.get("is_striker") or p.get("is_non_striker") or p.get("is_out"):
                icon = "❌" if p.get("is_out") else "🏏"
                sr   = (p["runs"] / p["balls_faced"] * 100) if p.get("balls_faced", 0) > 0 else 0.0
                bat_lines += f"  {icon} {p['name'][:13]}  <b>{p.get('runs',0)}</b>({p.get('balls_faced',0)})  SR:{sr:.0f}\n"
        if bat_lines:
            text += f"\n🏏 <i>Batting</i>\n{bat_lines}"
        ...
        for p in team.get("players", []):
            if p.get("balls_bowled", 0) > 0:
                ...
                bowl_lines += (
                    f"  🎯 {p['name'][:13]}  {p_ov}.{p_bl}Ov  "
                    f"<b>{p.get('conceded',0)}R/{p.get('wickets',0)}W</b>  Eco:{eco:.1f}\n"
                    f"{spell_str}\n" if spell_str else
                    f"  🎯 {p['name'][:13]}  {p_ov}.{p_bl}Ov  "
                    f"<b>{p.get('conceded',0)}R/{p.get('wickets',0)}W</b>  Eco:{eco:.1f}\n"
                )
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        exp_next   = (
            f"⬆️ Next: <b>{next_level_name}</b> · Need <b>{exp_needed}</b> EXP"
            if next_level_name else "⬆️ <b>MAX LEVEL!</b> 👑"
        )
        ...
        stats_text = (
            ...
            f"🏅 <b>{level}</b>  ·  ⭐ EXP: <b>{exp}</b>\n"
            f"{exp_next}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🏏 <b>BATTING</b>  ·  {bat_title}\n"
            f"🔸 HS: <b>{hs_runs}</b>({hs_balls}b)  Runs: <b>{total_runs}</b>  Avg: <b>{avg:.1f}</b>  SR: <b>{sr:.1f}</b>\n"
            f"🔸 6s: <b>{user_data.get('total_6s', 0)}</b>💥  4s: <b>{user_data.get('total_4s', 0)}</b>🔷  "
            f"100s: <b>{user_data.get('centuries', 0)}</b>  50s: <b>{user_data.get('half_centuries', 0)}</b>  🦆: <b>{user_data.get('ducks', 0)}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🥎 <b>BOWLING</b>  ·  {bowl_title}\n"
            f"🔹 Wkts: <b>{user_data.get('wickets', 0)}</b>  HT: <b>{user_data.get('hat_tricks', 0)}</b>  "
            f"Ovs: <b>{overs}.{rem_balls}</b>  Eco: <b>{eco:.2f}</b>\n"
            f"🔹 Best Spell: {bs_str}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🏆 Solo: <b>{user_data.get('solo_matches', 0)}</b>  Team: <b>{user_data.get('team_matches', 0)}</b>  MOTM: <b>{user_data.get('motm', 0)}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━"
        )
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_over_complete:
        ...
        if game.get("mode") == "TEAM":
            # Record this over's spell for the bowler before rotating
            spell_r = bowler.get("conceded", 0) - bowler.get("_spell_start_conceded", 0)
            spell_w = bowler.get("wickets",  0) - bowler.get("_spell_start_wkts",    0)
            bowler.setdefault("spells", []).append(f"{spell_r}R/{spell_w}W")
            bowler["_spell_start_conceded"] = bowler.get("conceded", 0)
            bowler["_spell_start_wkts"]     = bowler.get("wickets",  0)
            await update_best_spell(bowler["id"], spell_w, spell_r)
            swap_strike(game)
            ...
            # SOLO mode — record spell and update best spell in DB
            solo_spell_r = bowler.get("conceded", 0) - bowler.get("_spell_start_conceded", 0)
            solo_spell_w = bowler.get("wickets",  0) - bowler.get("_spell_start_wkts",    0)
            bowler["_spell_start_conceded"] = bowler.get("conceded", 0)
            bowler["_spell_start_wkts"]     = bowler.get("wickets",  0)
            await update_best_spell(bowler["id"], solo_spell_w, solo_spell_r)
            await trigger_full_scorecard_message(context, chat_id, game)
            await context.bot.send_message(chat_id, spell_text, parse_mode="HTML")
            game["balls_bowled"]           = 0
            game["special_used_this_over"] = False
