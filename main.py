...
def set_afk_timer(context, chat_id, user_id, role):
    ...
    if game.get('mode') == 'TEAM':
        ...
        context.job_queue.run_once(afk_warning_start, 1, data={'chat_id': chat_id, 'user_id': user_id, 'role': role}, name=f"afk1_{chat_id}")
...
async def afk_warning_start(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id, f"⚠️ <a href='tg://user?id={user_id}'>{player['name']}</a>, it is your turn! You have <b>60 seconds</b> to play. ⏳", parse_mode='HTML')
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == 'private':
        ...
        welcome_private = (
            ...
            "Experience real-time bowling, batting, yorkers, no-balls, free hits, and dynamic milestones just like the real game.\n\n"
            "Build your legacy, climb the leaderboards, and become a cricketing legend. Are you ready to take the strike?\n\n"
            "👉 <b>Join our official community at @eclplays to participate in tournaments and find players!</b>"
        )
        kb_private = [
