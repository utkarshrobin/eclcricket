...
HIT_COMMENTARY = {
    ...
    6: [
        ...
        "🎪 SIXXX! That ball cleared the boundary before the bowler even turned around!",
    ],
}
...
def generate_scorecard(game):
    ...
    for i, p in enumerate(players):
        ...
        overs, rems = divmod(p.get("balls_bowled", 0), 6)
        eco         = (p.get("conceded", 0) / p["balls_bowled"]) * 6 if p.get("balls_bowled", 0) > 0 else 0.0
        ...
        if p.get("is_out"):
            ...
            bat_emoji = "🏏"
            bat_label = "◀ ON PITCH — BATTING"
        else:
            bat_emoji = "🔸"
            bat_label = "Waiting"

        if state == "PLAYING" and i == bowler_idx:
            bowl_label = "◀ ON PITCH — BOWLING"
        else:
            bowl_label = ""

        text += (
            f"┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
            f"{bat_emoji} <b>{p['name']}</b>  <i>{bat_label}</i>\n"
            f"\n"
            f"  🏏 <b>Batting</b>\n"
            f"     ➜ Runs: <b>{runs}</b>  ·  Balls: <b>{balls_faced}</b>  ·  SR: <b>{sr:.1f}</b>\n"
            f"\n"
            f"  🥎 <b>Bowling</b>"
            f"{('  <i>' + bowl_label + '</i>') if bowl_label else ''}\n"
            f"     ➜ Wkts: <b>{p.get('wickets', 0)}</b>  ·  Runs: {p.get('conceded', 0)}"
            f"  ·  {overs}.{rems} Ov  ·  Eco: <b>{eco:.1f}</b>\n"
        )
        is_cur_bowler = state == "PLAYING" and i == bowler_idx
        solo_spells = list(p.get("bowling_spells", []))
        if is_cur_bowler and p.get("_spell_balls0") is not None:
            _lb = p.get("balls_bowled", 0) - p["_spell_balls0"]
            _lr = p.get("conceded", 0)      - p["_spell_runs0"]
            _lw = p.get("wickets", 0)        - p["_spell_wkts0"]
            if _lb > 0:
                solo_spells.append({"b": _lb, "r": _lr, "w": _lw, "live": True})
        if solo_spells:
            parts = []
            for idx, s in enumerate(solo_spells, 1):
                s_ov, s_bl = divmod(s["b"], 6)
                mark = "●" if s.get("live") else ""
                parts.append(f"(Sp{idx}{mark}: {s_ov}.{s_bl}ov · {s['r']}R · {s['w']}W)")
            text += f"     📋 {'  '.join(parts)}\n"
...
def generate_team_scorecard(game):
    ...
    for team_key, team_label in [("team_a", "🔴 TEAM A"), ("team_b", "🔵 TEAM B")]:
        ...
        p_overs, p_rem = divmod(played_balls, 6)
        total_ov       = p_overs + (p_rem / 6)
        ...
        text += (
            ...
            f"  ·  {p_overs}.{p_rem} ov  ·  RR: <b>{rr:.2f}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )
        ...
        bat_players = [
            ...
            if p.get("balls_faced", 0) > 0
            or p.get("is_striker")
            or p.get("is_non_striker")
            or p.get("is_out")
        ]
        if bat_players:
            text += "\n🏏 <b>BATTING</b>\n"
            for p in bat_players:
                ...
                sr   = (runs / bf * 100) if bf > 0 else 0.0
                ...
                text += (
                    ...
                    f"       ➜ <b>{runs}</b> runs  ({bf} balls)  SR: <b>{sr:.1f}</b>"
                    f"  ·  4s: {fours}  6s: {sixes}\n"
                )
            text += "\n"
        ...
        if bowl_players:
            ...
            for p in bowl_players:
                ...
                eco = (p.get("conceded", 0) / p["balls_bowled"]) * 6
                is_cur = is_playing and p.get("id") == current_bowler.get("id")
                if is_cur:
                    bowl_emoji = "🎯"
                    bowl_note  = "  <i>◀ bowling</i>"
                else:
                    bowl_emoji = "🥎"
                    bowl_note  = ""
                text += (
                    f"\n  {bowl_emoji}  <b>{p['name']}</b>{bowl_note}\n"
                    f"       ➜ <b>{p.get('wickets', 0)}W</b>  ·  {p.get('conceded', 0)}R"
                    f"  ·  {p_ov}.{p_bl} ov  ·  Eco: <b>{eco:.1f}</b>\n"
                )
...
async def _send_ranking(query, title: str, field: str, label: str, fmt=None):
    if users_col is None:
        await query.edit_message_text("❌ Database not connected.")
        return
    docs = await users_col.find({field: {"$gt": 0}}).to_list(length=50)
    random.shuffle(docs)
    ...
    if not top:
        ...
        for i, p in enumerate(top):
            ...
            lines += f"{medal} {name} — <b>{value}</b> {label}\n"
...
async def _send_sr_ranking(query):
    ...
    if not top:
        ...
        for i, p in enumerate(top):
            ...
            medal = medals[i] if i < 3 else f"<b>#{i+1}</b>"
            lines += f"{medal} {name} — <b>SR: {sr:.1f}</b>\n"
    back_kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="rank_main")]])
    await query.edit_message_text(
        f"🏆 <b>⚡ STRIKE RATE RANKING</b> (min 20 balls)\n━━━━━━━━━━━━━━━━━\n{lines}\n━━━━━━━━━━━━━━━━━\n🏏 <i>Ties broken randomly — keep playing!</i>",
        reply_markup=back_kb,
        parse_mode="HTML",
    )
...
async def _send_most_runs_in_match_ranking(query):
    if users_col is None:
        await query.edit_message_text("❌ Database not connected.")
        return
    docs = await users_col.find({"highest_score.runs": {"$gt": 0}}).to_list(length=50)
    random.shuffle(docs)
    ...
    if not top:
        ...
        for i, p in enumerate(top):
            ...
            medal = medals[i] if i < 3 else f"<b>#{i+1}</b>"
            lines += f"{medal} {name} — <b>{runs} runs</b> ({balls} balls)\n"
    back_kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="rank_main")]])
    await query.edit_message_text(
