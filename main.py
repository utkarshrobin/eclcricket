...
def set_afk_timer(context, chat_id, user_id, role):
    ...
    if game.get('mode') == 'TEAM':
        ...
        # Solo Mode: 60s total timer as per requirements
...
async def afk_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # If the batter was eliminated, move to the next valid one
        game['batter_idx'] = min(game['batter_idx'], len(game['players']) - 1)
        
    # Pick a random bowler that isn't the batter
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Restrict selection to Host or Batting Team Captain
    ...
    if game['waiting_for'] == 'TEAM_OPENERS_BAT':
        if not game.get('striker'):
            ...
            # FIX: Used proper direct giphy link with robust try-except error handling
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Restrict selection to Host or Bowling Team Captain
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    keyboard = []
    # Updated logic for special ball probability in Solo mode
    if not game.get('special_used_this_over'):
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        ...
        if game['team_a']['captain'] and game['team_b']['captain']:
            ...
            # FIX: used robust direct media URL for Giphy and wrapped in try-except block
        ...
        # Special Ball Probabilities updated per user ledger
        if roll <= 60: # 60% WIDE
