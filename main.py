...
def get_level_info(exp):
    if exp < 1000: return "Newbie 🔰", 1000, "Pro ⚡"
    elif exp <= 5000: return "Pro ⚡", 5001, "Legendary 🌟"
    elif exp <= 8000: return "Legendary 🌟", 8001, "Unbeaten 👑"
    else: return "Unbeaten 👑", None, None

def get_user_level(exp):
    return get_level_info(exp)[0]
...
'''
  ...
  🥎 Bowl: <b>{p['wickets']}</b>W | {p['conceded']}R | {overs}.{balls} Ov (Eco: {eco:.1f})
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
'''
    return text
...
📈 <b>Required Run Rate (RRR):</b> {rrr:.2f}
━━━━━━━━━━━━━━━━━━━━━━
'''
...
🎉 <b>Batting team reaches the target via penalty!</b>", parse_mode="HTML")
            await process_team_innings_end(context, chat_id, game)
            return
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        stats_text = f'''📊 <b>{level_name} STATISTICS</b> 📊
═══════════════════════════
👤 <b>Name:</b> {user_data.get('first_name', 'Unknown')}
🆔 <b>ID:</b> <code>{user_data.get('user_id', 'Unknown')}</code>
🌟 <b>Current EXP:</b> {exp}
'''
        if req_exp:
            stats_text += f"🎯 <b>Next Level:</b> {next_lvl} (Need {req_exp - exp} more EXP)
"
        else:
            stats_text += f"🎯 <b>Next Level:</b> MAXED OUT 👑
"
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📊 <b>View our top performers!</b>
...
async def spamfree_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game["spamfree"] = True
    game["state"] = "PLAYING"
    await update.message.reply_text("🛡️ <b>SPAM-FREE MODE ACTIVATED!</b> Bowlers cannot bowl the same delivery more than twice in a row.
...
async def spamfree_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    game["spamfree"] = False
    game["state"] = "PLAYING"
    await context.bot.send_message(chat_id, "⏳ Time is up! ⚠️ <b>SPAM IS ALLOWED.</b>
...
👉 <a href='tg://user?id={batter['id']}'>{batter['name']}</a>, type {hit_opts} to hit! 🏏👇")
        ...
        if is_weekly and not top_batters and not top_bowlers:
            try: await query.edit_message_text("⏳ <b>Still fetching data...</b> Play some matches to get on the board!", parse_mode="HTML")
            except: pass
            return
...
"
        ...
        try: await query.edit_message_text("📊 <b>View our top performers!</b>
Select a leaderboard below:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
        except: pass
    ...
    if chat_type == "private":
        ...
        if game.get("mode") == "TEAM" and game.get("spamfree"):
            ...
            if len(last_balls) >= 2 and last_balls[-1] == val and last_balls[-2] == val:
                await update.message.reply_text("⚠️ <b>SPAM FREE MODE ACTIVE:</b> You cannot bowl the same delivery more than 2 times in a row! Choose a different delivery.", parse_mode="HTML")
                return
