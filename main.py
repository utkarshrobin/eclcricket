...
# Keep your bot token hardcoded here as preferred for your current deployment setup
    ...
    if update.message.reply_to_message:
        ...
        if mentions:
            ...
            if e.type == "text_mention":
                target_player = next((p for p in game[team_key]["players"] if p["id"] == e.user.id), None)
            else:
                t_username = update.message.text[e.offset:e.offset+e.length].lstrip("@").lower()
                target_player = next((p for p in game[team_key]["players"] if p.get("username") == t_username), None)
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        ...
        lock_msg = await context.bot.send_message(chat_id, "⚡", parse_mode="HTML")
