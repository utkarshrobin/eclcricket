...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        bowl_title = get_bowling_title(eco) if balls_bowled > 0 else "N/A"
        ...
        next_lv_line = (
            f"⬆️ <b>Next Level:</b> {next_level_name} (Need <b>{exp_needed}</b> more EXP)"
            if next_level_name
            else "⬆️ <b>Level:</b> MAX LEVEL REACHED! 👑"
        )
        stats_text += (
            f"📌 <b>OTHER IMP STATS</b>\n"
            f"🏏 <b>Batting Title:</b> {bat_title}\n"
            f"🥎 <b>Bowling Title:</b> {bowl_title}\n"
            f"⭐ <b>Current Level:</b> {level}\n"
            f"💠 <b>Total EXP:</b> {exp}\n"
            f"{next_lv_line}\n"
            f"═══════════════════════════"
        )
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_over_complete:
        ...
        if game.get("state") == "PLAYING" and game.get("waiting_for") == "PROCESSING_BATTER":
            game["waiting_for"] = "BOWLER"
        # Chase pressure message — team mode 2nd innings, >20 runs still needed
        if (
            game.get("mode") == "TEAM"
            ...
            balls_total = game["bowling_team_ref"]["balls_bowled"]
            balls_in_over = balls_total % 6
            balls_left_in_over = (6 - balls_in_over) if balls_in_over > 0 else 0
            if runs_needed > 20 and balls_left_in_over > 0:
                striker = game.get("striker")
                if striker:
                    s_balls = striker.get("balls_faced", 0)
                    s_sr = (striker.get("runs", 0) / s_balls * 100) if s_balls > 0 else 0
                    bat_title = get_batting_title(s_sr)
                    rpb = runs_needed / balls_left_in_over
                    await context.bot.send_message(
                        chat_id,
                        f"🔥 <b>PRESSURE ZONE!</b> 🔥\n"
                        f"⚡ <b>{runs_needed} runs needed</b> from "
                        f"<b>{balls_left_in_over} ball{'s' if balls_left_in_over != 1 else ''}</b> "
