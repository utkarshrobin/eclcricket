...
async def teams_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private":
        return
    game = context.bot_data.get(chat_id)
    if not game or game.get("mode") != "TEAM" or game.get("state") in ["NOT_PLAYING", "TEAM_SETUP_HOST", "TEAM_JOINING", "TEAM_CAPTAINS"]:
        await update.message.reply_text("❌ No active team match right now!")
        return
    roster = generate_teams_message(game)
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        stats_text = (
            ...
            f"🆔 <code>#{user_data.get('user_id', '—')}</code>  🏅 <b>{level}</b>\n"
            f"\n"
            f"🏏 <b>BATTING</b>  ·  <i>{bat_title}</i>\n"
            f"{SSEP}\n"
            f"🔝 Highest  →  <b>{hs_runs}</b> ({hs_balls}b)\n"
            f"🏃 Runs     →  <b>{total_runs:,}</b>\n"
            f"📊 Avg/SR   →  <b>{avg:.1f}</b> / <b>{sr:.1f}</b>\n"
            f"💥 6s / 4s  →  <b>{user_data.get('total_6s', 0)}</b> / <b>{user_data.get('total_4s', 0)}</b>\n"
            ...
            f"🎩 Hat-Trick →  <b>{user_data.get('hat_tricks', 0)}</b>\n"
            f"📐 Ovs/Eco  →  <b>{overs}.{rem_balls}</b> / <b>{eco:.2f}</b>\n"
            f"⚡ Best Spell →  {bs_str}\n"
            f"\n"
            f"🏆 <b>CAREER</b>\n"
            f"{SSEP}\n"
            f"🎮 Solo → <b>{user_data.get('solo_matches', 0)}</b>  👥 Team → <b>{user_data.get('team_matches', 0)}</b>\n"
            f"🏅 MOTM →  <b>{user_data.get('motm', 0)}</b>\n"
            f"⭐ EXP  →  <b>{exp:,}</b>\n"
            f"↑ {exp_next}\n"
            f"{SEP}"
        )
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "solo_game":
        ...
        try:
            ...
            pass
        # Announce in group (no instructions — those go to bowler DM only)
        await context.bot.send_message(
            grp_id,
            f"🎯 <b>{bowler['name']}</b> is attempting a <b>YORKER</b>! 🎯",
            parse_mode="HTML",
        )
        # Tell the bowler in DM what to do
        try:
            await context.bot.send_message(
                bowler["id"],
                "🎯 <b>Yorker activated!</b>\nNow send your delivery number (1-6) to complete it!",
                parse_mode="HTML",
            )
        except Exception:
            pass
    ...
    if chat_type == "private":
        ...
        if reg_state and reg_state not in ("confirm",):
            ...
            if reg_state == "team_name":
                ...
                return
        # Not in a registration flow — check if this is a bowler sending their delivery
        if user_input.isdigit() and int(user_input) in range(1, 7):
            await handle_bowler_dm(context, update.effective_user.id, int(user_input))
...
async def process_solo_ball(context, chat_id, game, shot):
    ...
    if is_yorker and is_out:
        ...
        await context.bot.send_message(
            chat_id,
            f"❌ <b>{batter['name']}</b> got out at <b>{batter.get('runs', 0)}</b> ({batter.get('balls_faced', 0)} balls) 🏏\n"
            f"🥎 Bowled by <b>{bowler['name']}</b>",
            parse_mode="HTML",
        )
        await asyncio.sleep(0.3)
...
async def process_team_ball(context, chat_id, game, shot):
    ...
    if is_yorker and is_out:
        ...
        await context.bot.send_message(
            chat_id,
            f"❌ <b>{striker['name']}</b> got out at <b>{striker.get('runs', 0)}</b> ({striker.get('balls_faced', 0)} balls) 🏏\n"
            f"🥎 Bowled by <b>{bowler['name']}</b>",
            parse_mode="HTML",
        )
        await asyncio.sleep(0.3)
