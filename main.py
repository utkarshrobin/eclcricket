...
📈 <b>PENALTY:</b> +5 Runs to Batting Team!", parse_mode="HTML")
            await process_team_innings_end(context, chat_id, game)
            return
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if exp < 1000: req_exp = 1000 - exp; next_lvl = "Pro ⚡"
        elif exp <= 5000: req_exp = 5001 - exp; next_lvl = "Legendary 🌟"
        elif exp <= 8000: req_exp = 8001 - exp; next_lvl = "Unbeaten 👑"
        else: req_exp = 0; next_lvl = "MAX"
...
"
        stats_text += f"🔸 <b>Current EXP:</b> {exp}
"
        if next_lvl != "MAX": stats_text += f"🔸 <b>Next Level:</b> {next_lvl} (Needs {req_exp} more EXP)
"
        else: stats_text += f"🔸 <b>Next Level:</b> MAX LEVEL REACHED 🏆
"
        stats_text += f"┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
"
