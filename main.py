...
# --- MONGODB SETUP ---
# Remember to run: pip install motor
...
if MONGO_URI:
    # Added tlsCAFile=certifi.where() to fix SSL verification bugs on certain machines
    db_client = AsyncIOMotorClient(MONGO_URI, tlsCAFile=certifi.where())
    db = db_client['cricket_bot']
    users_col = db['users']
else:
    users_col = None
    print("WARNING: MONGO_URI is empty. Stats will not be saved to database.")
...
# --- HELPER FUNCTIONS ---
async def commit_match_stats(game):
    if not users_col: return
    
    mode = game.get('mode')
    players = []
    if mode == 'TEAM':
        players = game.get('team_a', {}).get('players', []) + game.get('team_b', {}).get('players', [])
    elif mode == 'SOLO':
        players = game.get('players', [])
        
    # Determine MOTM locally to award stats
    potm_player = None
    best_score = -999
    for p in players:
    ...
    for p in players:
        ...
        doc = await users_col.find_one({'_id': pid})
        if not doc:
            doc = {
                '_id': pid, 'name': p.get('name', 'Unknown'), 'username': p.get('username'),
                'matches_solo': 0, 'matches_team': 0, 'runs': 0, 'balls_faced': 0,
                'highest_runs': 0, 'highest_balls': 0, 'sixes': 0, 'fours': 0,
                'centuries': 0, 'half_centuries': 0, 'ducks': 0,
                'balls_bowled': 0, 'runs_conceded': 0, 'wickets': 0, 'motm': 0
            }
            
        if mode == 'SOLO': doc['matches_solo'] = doc.get('matches_solo', 0) + 1
        elif mode == 'TEAM': doc['matches_team'] = doc.get('matches_team', 0) + 1
        
        runs = p.get('runs', 0)
        doc['runs'] = doc.get('runs', 0) + runs
        doc['balls_faced'] = doc.get('balls_faced', 0) + p.get('balls_faced', 0)
        doc['sixes'] = doc.get('sixes', 0) + p.get('sixes', 0)
        doc['fours'] = doc.get('fours', 0) + p.get('fours', 0)
        
        if runs == 0 and p.get('is_out', False):
            doc['ducks'] = doc.get('ducks', 0) + 1
            
        if runs >= 100: doc['centuries'] = doc.get('centuries', 0) + 1
        elif runs >= 50: doc['half_centuries'] = doc.get('half_centuries', 0) + 1
            
        doc['balls_bowled'] = doc.get('balls_bowled', 0) + p.get('balls_bowled', 0)
        doc['runs_conceded'] = doc.get('runs_conceded', 0) + p.get('conceded', 0)
        doc['wickets'] = doc.get('wickets', 0) + p.get('wickets', 0)
        
        if potm_player and pid == potm_player['id']:
            doc['motm'] = doc.get('motm', 0) + 1
            
        if runs > doc.get('highest_runs', 0):
            doc['highest_runs'] = runs
            doc['highest_balls'] = p.get('balls_faced', 0)
            
        doc['name'] = p.get('name', doc.get('name'))
        doc['username'] = p.get('username', doc.get('username'))
        
        await users_col.update_one({'_id': pid}, {'$set': doc}, upsert=True)
...
async def afk_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    batter_id = game['players'][game['batter_idx']]['id']
    game['players'] = [p for p in game['players'] if p['id'] != user_id]
        
    if len(game['players']) < 2:
        game['state'] = 'NOT_PLAYING'
        await commit_match_stats(game)
        await context.bot.send_message(chat_id, "Not enough players left! Match abandoned. 🛑", parse_mode='HTML')
        return
        
    # Re-evaluate batter index based on remaining players
    try:
        new_batter_idx = next(i for i, p in enumerate(game['players']) if p['id'] == batter_id)
        game['batter_idx'] = new_batter_idx
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not users_col:
        await update.message.reply_text("❌ Database is not configured. User stats are unavailable.")
        return
    ...
    try:
        if update.message.reply_to_message:
            ...
            username_query = context.args[0].lstrip('@').lower()
            doc = await users_col.find_one({'username': username_query})
            if not doc:
                 ...
                 for entity in update.message.entities:
                      if entity.type == 'text_mention':
                           ...
                           break
                 if not target_user:
                      await update.message.reply_text("❌ User not found in database by username!")
                      return
        else:
            target_user = update.effective_user

        if target_user:
             # Ensure users have their username updated if they play a game without registering
             ...
             doc = await users_col.find_one({'_id': target_user.id})

        if not doc:
            await update.message.reply_text("❌ No stats found for this user. Play a match first!")
            return
            
        runs = doc.get('runs', 0)
        balls = doc.get('balls_faced', 0)
        sr = (runs / balls * 100) if balls > 0 else 0.0
        
        b_balls = doc.get('balls_bowled', 0)
        b_runs = doc.get('runs_conceded', 0)
        eco = (b_runs / b_balls * 6) if b_balls > 0 else 0.0
        overs_str = f"{b_balls // 6}.{b_balls % 6}"
        
        text = f"📊 <b>PLAYER STATS: {doc.get('name', 'Player')}</b> 📊\n"
        text += f"👤 <b>ID:</b> <code>{doc.get('_id')}</code>\n"
        if doc.get('username'):
            text += f"📛 <b>Username:</b> @{doc['username']}\n"
        text += "══════════════════════\n"
        text += "🏏 <b>BATTING STATS</b>\n"
        text += f"🔸 <b>Total Runs:</b> {runs}\n"
        text += f"🔸 <b>Highest Score:</b> {doc.get('highest_runs', 0)} ({doc.get('highest_balls', 0)} balls)\n"
        text += f"🔸 <b>Strike Rate:</b> {sr:.1f}\n"
        text += f"🔸 <b>Sixes (6s):</b> {doc.get('sixes', 0)} | <b>Fours (4s):</b> {doc.get('fours', 0)}\n"
        text += f"🔸 <b>Centuries (100s):</b> {doc.get('centuries', 0)} | <b>Fifties (50s):</b> {doc.get('half_centuries', 0)}\n"
        text += f"🔸 <b>Ducks (0s):</b> {doc.get('ducks', 0)}\n"
        text += "══════════════════════\n"
        text += "🥎 <b>BOWLING STATS</b>\n"
        text += f"🔹 <b>Wickets:</b> {doc.get('wickets', 0)}\n"
        text += f"🔹 <b>Overs Bowled:</b> {overs_str}\n"
        text += f"🔹 <b>Economy Rate:</b> {eco:.1f}\n"
        text += f"🔹 <b>Runs Conceded:</b> {b_runs}\n"
        text += "══════════════════════\n"
        text += "🏆 <b>ADDITIONAL STATS</b>\n"
        text += f"🏅 <b>Player of the Match (MOTM):</b> {doc.get('motm', 0)}\n"
        text += f"🎮 <b>Solo Matches:</b> {doc.get('matches_solo', 0)} | <b>Team Matches:</b> {doc.get('matches_team', 0)}\n"
        
        await update.message.reply_text(text, parse_mode='HTML')

    except Exception as e:
        # If the database connection drops or fails, the bot will actually tell you now!
        await update.message.reply_text(f"⚠️ <b>Database Error:</b> Could not fetch stats right now. \n\n<i>Error details: {str(e)}</i>", parse_mode='HTML')
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == 'private':
        ...
        welcome_private = (
            ...
            "Experience real-time bowling, batting, yorkers, no-balls, free hits, and dynamic milestones just like the real game. Build your legacy, climb the leaderboards, and become a cricketing legend.\n\n"
            "Are you ready to take the strike? Join our official community to participate in tournaments and find players!"
        )
        kb_private = [
...
async def create_team_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Updated to 10 seconds per request
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_user_playing_anywhere(context, target_user.id):
        await update.message.reply_text(f"❌ {target_user.first_name} is already in a match or queue in another group! They can only play one game at a time.")
        return
    ...
    username = target_user.username.lower() if target_user.username else None
    new_player = {'id': target_user.id, 'name': target_user.first_name, 'username': username, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False, 'sixes': 0, 'fours': 0}
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_user_playing_anywhere(context, user.id):
        await update.message.reply_text(f"❌ {user.first_name}, you are already in a match or queue in another group! You can only play one game at a time.")
        return
    ...
    game['players'].append({'id': user.id, 'name': user.first_name, 'username': username, 'runs': 0, 'conceded': 0, 'wickets': 0, 'balls_bowled': 0, 'balls_faced': 0, 'sixes': 0, 'fours': 0})
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'solo_game':
        ...
        game['team_a']['players'].append({'id': user_id, 'name': update.effective_user.first_name, 'username': username, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False, 'sixes': 0, 'fours': 0})
        ...
        username = update.effective_user.username.lower() if update.effective_user.username else None
        game['team_b']['players'].append({'id': user_id, 'name': update.effective_user.first_name, 'username': username, 'runs': 0, 'balls_faced': 0, 'wickets': 0, 'conceded': 0, 'balls_bowled': 0, 'is_out': False, 'sixes': 0, 'fours': 0})
        ...
        if game['team_a']['captain'] and game['team_b']['captain']:
            ...
            toss_gif = "https://media.giphy.com/media/3Jt57qFHMiCeHMuJ6O/giphy.gif"
            caption_msg = f"🪙 <b>TOSS TIME!</b>\n<a href='tg://user?id={cap_id}'>{cap_name}</a>, call the toss!"
            
            try:
                await context.bot.send_animation(chat_id, animation=toss_gif, caption=caption_msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
            except Exception:
                await context.bot.send_message(chat_id, text=caption_msg, reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML')
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler('userstats', userstats_command))  # New stats command
