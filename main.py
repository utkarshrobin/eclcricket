    ...
    if query.data == "solo_game":
        ...
        if context.bot_data.get(chat_id, {}).get("state") in ["JOINING", "PLAYING"]:
            try:
                ...
                pass
            return
        # Only the /start initiator can pick the spell length
        if user_id != game.get("start_initiator_id"):
            try:
                await query.answer("⚠️ Only the person who typed /start can set up the match!", show_alert=True)
            except Exception:
                pass
            return
        ...
        if context.bot_data[lock_key].locked():
            try:
                await query.answer("⚠️ Match is already being set up!", show_alert=True)
            except Exception:
                pass
            return
        async with context.bot_data[lock_key]:
            if context.bot_data.get(chat_id, {}).get("state") in ["JOINING", "PLAYING"]:
                try:
                    await query.answer("❌ A match is already active!", show_alert=True)
                except Exception:
                    pass
                return
...
async def potd_daily_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    player = await users_col.find_one({}, skip=skip)
    if not player:
        return
    ...
    # Save to bot_data so /potd command can retrieve it
