...
def get_potd_badge(player_id: int, context) -> str:
    potd = context.bot_data.get("potd", {})
    if potd.get("user_id") and potd["user_id"] == player_id:
        return " 😶‍🌫️ POTD"
    return ""
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        next_lv_line = (
            f"⬆️  Next Level : {next_level_name}  |  Need <b>{exp_needed}</b> more EXP"
            if next_level_name
            else "⬆️  Level : <b>MAX LEVEL REACHED!</b> 👑"
        )
        ...
        stats_text = (
            ...
            f"╚══════════════════════════╝\n"
            f"\n"
            f"👤  <b>{user_data.get('first_name', 'Unknown')}</b>   <code>#{user_data.get('user_id', '—')}</code>\n"
            f"🏅  <b>{level}</b>\n"
            f"\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🏏  <b>BATTING</b>\n"
            ...
            f"  🔸  Batting Avg     :  <b>{avg:.2f}</b>\n"
            f"  🔸  Strike Rate     :  <b>{sr:.2f}</b>\n"
            ...
            f"  🔹  Overs Bowled    :  <b>{overs}.{rem_balls}</b>\n"
            f"  🔹  Economy Rate    :  <b>{eco:.2f}</b>\n"
            f"\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🏆  <b>MATCHES &amp; AWARDS</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            f"\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📌  <b>OTHER IMP STATS</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"  🏏  Batting Title   :  <b>{bat_title}</b>\n"
            f"  🥎  Bowling Title   :  <b>{bowl_title}</b>\n"
            f"  ⭐  EXP             :  <b>{exp}</b>\n"
            f"  {next_lv_line}\n"
            f"\n"
            f"╔══════════════════════════╗\n"
            f"   Use /leaderboard to rank up!\n"
            f"╚══════════════════════════╝"
        )

        stats_img = "https://res.cloudinary.com/dxgfxfoog/image/upload/v1777818873/file_00000000fa6871fa8d9b30faff9899ae_hbyn9j.png"
        try:
            await msg.reply_photo(photo=stats_img)
        except Exception:
            pass
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    batter_badge = get_potd_badge(batter["id"], context)
    bowler_badge = get_potd_badge(bowler["id"], context)
    if dm_sent:
        group_text = (
            f"{free_hit_tag}📊 <b>Status:</b>\n"
            f"🏏 <b>Batter:</b> {batter['name']}{batter_badge} ({batter.get('runs', 0)} off {batter.get('balls_faced', 0)})\n"
            f"🥎 <b>Bowler:</b> {bowler['name']}{bowler_badge} (Over: {over_info})\n\n"
            f"👉 <a href='tg://user?id={bowler['id']}'>{bowler['name']}{bowler_badge}</a>, check your DM to bowl! 🤫🥎"
        )
        ...
        group_text = (
            ...
            f"🏏 <b>Batter:</b> {batter['name']}{batter_badge} ({batter.get('runs', 0)} off {batter.get('balls_faced', 0)})\n"
            f"🥎 <b>Bowler:</b> {bowler['name']}{bowler_badge} (Over: {over_info})\n\n"
            f"⚠️ <a href='tg://user?id={bowler['id']}'>{bowler['name']}{bowler_badge}</a>, I couldn't DM you! "
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        ...
        game["waiting_for"]  = "PROCESSING_BOWLER"   # batter blocked until group msg sent
        ...
        batter_badge_dm = get_potd_badge(batter["id"], context)
        await send_media_safely(
            context, group_id, MEDIA["batter_turn"],
            f"🚨 Ball bowled! 🥎💨\n👉 <a href='tg://user?id={batter['id']}'>{batter['name']}{batter_badge_dm}</a>, type {hit_opts} to hit! 🏏👇",
        )
        # Only open batter input AFTER the group ping is fully sent
        game["waiting_for"] = "BATTER"
        set_afk_timer(context, group_id, batter["id"], "BATTER")
        return
        ...
        if not survives:
            if is_free_hit:
                ...
                if batter.get("runs", 0) == 0:
                    ...
                    await context.bot.send_message(
                        chat_id,
                        f"😔 <b>{batter['name']}</b> retirement loading soon 😔",
                        parse_mode="HTML",
                    )
    ...
    if is_over_complete:
        ...
        if game.get("state") == "PLAYING" and game.get("waiting_for") == "PROCESSING_BATTER":
            game["waiting_for"] = "BOWLER"
        # Chase pressure — only in last 2 overs (≤12 total balls remaining)
        if (
            game.get("mode") == "TEAM"
            and game.get("innings") == 2
            and game.get("state") == "PLAYING"
        ):
            runs_needed      = game.get("target", 0) - game["batting_team_ref"]["score"]
            balls_bowled_tot = game["bowling_team_ref"]["balls_bowled"]
            target_balls     = game.get("target_overs", 0) * 6
            balls_remaining  = target_balls - balls_bowled_tot
            if 0 < balls_remaining <= 12 and runs_needed > 0:
                striker = game.get("striker")
                if striker:
                    # Use career SR from DB for accurate title
                    career_sr = 0
                    if users_col is not None:
                        try:
                            ud = await users_col.find_one(
                                {"user_id": striker["id"]},
                                {"total_runs": 1, "balls_faced": 1},
                            )
                            if ud and ud.get("balls_faced", 0) > 0:
                                career_sr = ud["total_runs"] / ud["balls_faced"] * 100
                        except Exception:
                            pass
                    bat_title = get_batting_title(career_sr)
                    rpb = runs_needed / balls_remaining
                    await context.bot.send_message(
                        chat_id,
                        f"🔥 <b>PRESSURE ZONE!</b> 🔥\n"
                        f"⚡ <b>{runs_needed} runs needed</b> from "
                        f"<b>{balls_remaining} ball{'s' if balls_remaining != 1 else ''}</b> "
                        f"(<b>{rpb:.1f} per ball</b>)\n\n"
                        f"🏏 <b>{striker['name']}</b> [{bat_title}] is on strike — can he pull it off? 😤💪",
                        parse_mode="HTML",
                    )
...
async def potd_daily_job(context: ContextTypes.DEFAULT_TYPE):
    if users_col is None or chats_col is None:
        return

    # Only consider players who have actually played at least 1 match,
    # then pick the one with the highest EXP earned — strictly the best performer
    eligible = await users_col.find(
        {"$or": [{"solo_matches": {"$gt": 0}}, {"team_matches": {"$gt": 0}}]}
    ).sort("exp", -1).limit(1).to_list(length=1)
    ...
    mention  = f"@{username}" if username else f"<b>{name}</b>"

    total_matches = player.get("solo_matches", 0) + player.get("team_matches", 0)
    ...
    text = (
        f"🌟✨ <b>PLAYER OF THE DAY</b> ✨🌟\n\n"
        f"🏏 Today's spotlight shines on:\n\n"
        f"👑 {mention}  [{level}]\n\n"
        f"📊 <b>Quick Stats:</b>\n"
        f"🏏 Runs: <b>{total_runs}</b>\n"
        f"🥎 Wickets: <b>{wickets}</b>\n"
        f"🎮 Matches: <b>{total_matches}</b>\n"
        f"⭐ EXP: <b>{exp}</b>\n\n"
        f"🎉 Congratulations! Keep smashing those boundaries! 💥🔥\n"
        f"📊 Check your stats with /userstats"
    )

    # Save so /potd command can show it
    from datetime import date
    today_str = date.today().isoformat()
    context.bot_data["potd"] = {
        "date": today_str,
        "name": name,
        "username": username,
    ...
    # Broadcast to all groups + pin if possible
    async for chat in chats_col.find({"type": {"$in": ["group", "supergroup"]}}):
        cid = chat["chat_id"]
        try:
            msg = await context.bot.send_message(cid, text, parse_mode="HTML")
            try:
                await context.bot.pin_chat_message(cid, msg.message_id, disable_notification=False)
            except Exception:
                pass
        except Exception:
            pass
...
async def potd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    potd = context.bot_data.get("potd")
    if not potd:
        await update.message.reply_text("🌟 No Player of the Day has been selected yet. Check back tonight at 10 PM! 🕙")
        return
    await update.message.reply_text(potd["text"], parse_mode="HTML")
...
if __name__ == "__main__":
    ...
    # ── Daily Player of the Day at 10:00 PM IST (16:30 UTC) ──────────────
    from datetime import time as _dt_time
    app.job_queue.run_daily(
        potd_daily_job,
        time=_dt_time(16, 30, 0),
        name="potd_daily",
    )
