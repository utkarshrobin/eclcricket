...
def get_user_level(exp):
    if exp < 1000: return "Newbie ð°"
    elif exp <= 5000: return "Pro â¡"
    elif exp <= 8000: return "Legendary ð"
    else: return "Unbeaten ð"

def get_level_info(exp):
    if exp < 1000: return "Newbie ð°", 1000, "Pro â¡"
    elif exp <= 5000: return "Pro â¡", 5001, "Legendary ð"
    elif exp <= 8000: return "Legendary ð", 8001, "Unbeaten ð"
    else: return "Unbeaten ð", None, None
...
async def init_user_db(user_id, first_name, username):
    ...
    if not user:
        await users_col.insert_one({
            ...
            "exp": 0, "weekly_runs": 0, "weekly_wickets": 0, "weekly_conceded": 0, "weekly_balls_bowled": 0, "weekly_balls_faced": 0,
        ...
        if user.get("first_name") != first_name or user.get("username") != username:
            await users_col.update_one({"user_id": user_id}, {"$set": {"first_name": first_name, "username": username}})
...
async def commit_player_stats(game):
    ...
    for p in players:
        ...
        updates = {
            ...
            "weekly_runs": runs, "weekly_balls_faced": balls_faced, "weekly_balls_bowled": p.get("balls_bowled", 0),
            "weekly_conceded": p.get("conceded", 0), "weekly_wickets": p.get("wickets", 0)
        }
...
"
    for p in game["players"]:
        ...
        text += f"ð¤ <b>{p['name']}</b>
  ð Bat: <b>{p['runs']}</b> ({p['balls_faced']})
"
        text += f"  ð¥ Bowl: <b>{p['wickets']}</b>W | {p['conceded']}R | {overs}.{balls} Ov (Eco: {eco:.1f})
ââââââââââââââââââââââ
"
    return text

def generate_team_scorecard(game):
    text = "ð <b>MATCH SCORECARD</b> ð
ââââââââââââââââââââââ
"
    ...
    if game.get("innings") == 2 and game.get("state") != "TEAM_FINISHED":
        ...
        text += f"ð¯ <b>Target:</b> {target} | Need <b>{max(0, runs_needed)}</b> runs in <b>{balls_rem}</b> balls.
"
        text += f"ð <b>Required Run Rate (RRR):</b> {rrr:.2f}
ââââââââââââââââââââââ
"
        
    for team_key, team_name in [("team_a", "ð´ TEAM A"), ("team_b", "ðµ TEAM B")]:
        ...
        text += f"ðï¸ <b>{team_name}</b> â <b>{team['score']}/{team['wickets']}</b> <i>({played_overs}.{rem_balls} Ov)</i> | <b>RR: {rr:.2f}</b>
...
"
        ...
        for p in team["players"]:
            if p["balls_faced"] > 0 or p.get("is_striker") or p.get("is_non_striker") or p.get("is_out"):
                status = "â" if p.get("is_out") else "ð"
                sr = (p["runs"] / p["balls_faced"] * 100) if p["balls_faced"] > 0 else 0.0
                batters_txt += f"  {status} {p['name'][:12]} â <b>{p['runs']}</b> ({p['balls_faced']}) [SR: {sr:.1f}]
"
...
"
        ...
        for p in team["players"]:
            if p["balls_bowled"] > 0:
                ...
                bowlers_txt += f"  {p['name'][:12]} â {p_ov}.{p_bl} Ov | <b>{p['conceded']}R</b> | <b>{p['wickets']}W</b> [Eco: {eco:.1f}]
"
        if bowlers_txt: text += f"ð¥ <i>Bowlers:</i>
{bowlers_txt}"
    ...
    if game["state"] == "TEAM_FINISHED":
        ...
        if team_a_score > team_b_score:
            if game["batting_team_ref"] == game["team_a"] and game["innings"] == 2:
                ...
                result_str = f"ð <b>Team A ð´ WINS by {wickets_left} wickets!</b>
"
            else:
                run_diff = team_a_score - team_b_score
                result_str = f"ð <b>Team A ð´ WINS by {run_diff} runs!</b>
"
        elif team_b_score > team_a_score:
             if game["batting_team_ref"] == game["team_b"] and game["innings"] == 2:
                wickets_left = (len(game["team_b"]["players"]) - 1) - game["team_b"]["wickets"]
                result_str = f"ð <b>Team B ðµ WINS by {wickets_left} wickets!</b>
"
             else:
                run_diff = team_b_score - team_a_score
                result_str = f"ð <b>Team B ðµ WINS by {run_diff} runs!</b>
"
        else:
            result_str = "ð¤ <b>IT'S A TIE!</b> ð¤
"

        text += result_str + "ââââââââââââââââââââââ
"
    return text
...
ð <b>PLAYER OF THE MATCH: <a href='tg://user?id={best_player['id']}'>{best_player['name']}</a></b> ð
Here is your reward, take this ð
"
    return ""

def generate_teams_message(game):
    text = "ðï¸ <b>TEAMS ROSTER</b> ðï¸
...
"
    ...
    for team_key, team_dict in [("team_a", game.get("team_a", {})), ("team_b", game.get("team_b", {}))]:
        team_name = "ð´ <b>TEAM A</b>" if team_key == "team_a" else "ðµ <b>TEAM B</b>"
        text += f"{team_name}
"
        
        for i, p in enumerate(team_dict.get("players", []), 1):
            cap = " (C) ð" if team_dict.get("captain") == p["id"] else ""
            status = ""
            if is_playing:
...
async def check_solo_winner_exp(game):
    if game.get("mode") == "SOLO" and len(game.get("players", [])) > 0:
        best_p = max(game["players"], key=lambda x: x["runs"])
        await update_user_db(best_p["id"], {"exp": 60})
...
async def afk_warning_start(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id, f"â ï¸ <a href='tg://user?id={user_id}'>{player['name']}</a>, it is your turn! You have <b>50 seconds</b> to play. â³", parse_mode="HTML")
...
async def afk_warning_30(context: ContextTypes.DEFAULT_TYPE):
    ...
    player = next((p for p in game["players"] if p["id"] == user_id), None)
    if not player: return
    await context.bot.send_message(chat_id, f"â ï¸ <a href='tg://user?id={user_id}'>{player['name']}</a>, HURRY UP! You only have <b>30 seconds</b> left to play! â°", parse_mode="HTML")
...
async def afk_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id, f"â³ <b>TIME'S UP!</b> {player['name']} was AFK for 60 seconds and has been ELIMINATED! â", parse_mode="HTML")
    ...
    if len(game["players"]) < 2:
        ...
        await context.bot.send_message(chat_id, "Not enough players left! Match abandoned. ð", parse_mode="HTML")
        return
    ...
    if game["batter_idx"] >= len(game["players"]):
        ...
        await commit_player_stats(game)
        game["state"] = "NOT_PLAYING"
        await context.bot.send_message(chat_id, "ð <b>MATCH FINISHED!</b> ð", parse_mode="HTML")
...
async def team_afk_warning_10(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not player: return
    await context.bot.send_message(chat_id, f"â ï¸ <a href='tg://user?id={user_id}'>{player['name']}</a>, you have been AFK! You have <b>50 more seconds</b> left to play. â³", parse_mode="HTML")
...
async def team_afk_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "BATTER":
        ...
        await context.bot.send_message(chat_id, f"â³ <b>TIME'S UP!</b> {player['name']} was AFK for 60 seconds! â
ð <b>PENALTY:</b> -5 Runs to the team and batter! They are OUT!", parse_mode="HTML")
        ...
        await context.bot.send_message(chat_id, f"ð Captain/Host, please select the next batter using <code>/batting [number]</code>.", parse_mode="HTML")
    elif role == "BOWLER":
        game["batting_team_ref"]["score"] += 5
        player["conceded"] += 5
        if game["innings"] == 2 and game["batting_team_ref"]["score"] >= game["target"]:
            await context.bot.send_message(chat_id, f"â³ <b>TIME'S UP!</b> {player['name']} timed out! â
ð <b>PENALTY:</b> +5 Runs to Batting Team!
ð <b>Batting team reaches the target via penalty!</b>", parse_mode="HTML")
            await process_team_innings_end(context, chat_id, game)
            return
...
async def queue_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("state") != "JOINING" or game.get("mode") != "SOLO":
        ...
        return
    await context.bot.send_message(chat_id, f"â³ <b>Queue is open!</b> Type /join to enter the match! There are 35 seconds left to join. (Total: {len(game['players'])}) ð", parse_mode="HTML")
...
async def auto_start_match(context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(game["players"]) >= 2:
        ...
        await context.bot.send_message(chat_id, "â³ <b>70 seconds are up! THE MATCH AUTO-STARTS NOW!</b> ð¨
Let's head to the pitch! ðï¸", parse_mode="HTML")
        await trigger_bowl(context, chat_id)
    else:
        game["state"] = "NOT_PLAYING"
        await context.bot.send_message(chat_id, "â³ <b>70 seconds are up, but there are not enough players!</b> Match setup abandoned. ð", parse_mode="HTML")
...
async def trigger_team_captains(context, chat_id, game):
    ...
    kb = [[InlineKeyboardButton("Team A Captain ð", callback_data="team_cap_a"), InlineKeyboardButton("Team B Captain ð", callback_data="team_cap_b")]]
    await context.bot.send_message(chat_id, "Who will lead the teams? Members click your team's button to become the Captain! â¡", reply_markup=InlineKeyboardMarkup(kb))
...
async def team_join_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(game["team_a"]["players"]) < 2 or len(game["team_b"]["players"]) < 2:
        ...
        await context.bot.send_message(chat_id, "â³ Time's up! But we need at least 2 players in each team! The queue is paused. â¸ï¸
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type != 'private':
        ...
        if current_time < cooldown:
            ...
            await update.message.reply_text(f"â³ Start command is under cooldown! Try again after {rem} seconds.")
            return
            ...
            try:
                ...
                if game and game["state"] == "PLAYING" and game.get("waiting_for") == "BOWLER":
                    ...
                    if bowler and update.effective_user.id == bowler["id"]:
                        keyboard = []
                        if not game.get("special_used_this_over"): keyboard.append([InlineKeyboardButton("ð¯ Try for yorker ð¯", callback_data=f"special_{group_id}")])
                        await update.message.reply_text(f"ð¥ <b>Your Turn to Bowl!</b>
Type 1-6 or Try for yorker! ð¤ð", reply_markup=InlineKeyboardMarkup(keyboard) if keyboard else None, parse_mode="HTML")
                        return
                    else:
                        await update.message.reply_text("It is not your turn to bowl right now! ð«ð")
                        return
            except ValueError: pass
                
        welcome_private = (
            "ð <b>PLAY LIVE CRICKET INSIDE TELEGRAM</b>

"
            "â¡ Real-time matches
"
            "ð Compete with friends
"
            "ð¯ Become LEGEND ð
...
"
        ...
        kb_private = [
            [InlineKeyboardButton("ADD IN GROUP TO PLAY â", url=f"https://t.me/{bot_info.username}?startgroup=true")],
            [InlineKeyboardButton("STATS ð", callback_data="dm_stats"), InlineKeyboardButton("Support Group ð¬", url="https://t.me/eclplays")],
            [InlineKeyboardButton("Contact Developer ð¨âð»", url="https://t.me/xrztz")]
        ]
    ...
    if game.get("state") not in ["NOT_PLAYING", None, "TEAM_FINISHED"]:
        await update.message.reply_text("â A match is already active in this group! Finish it or ask an admin to /endmatch first.")
        return
        
    welcome_text = "Welcome to the <b>ELITE CRICKET BOT</b> Arena! ð
Join our official community at @eclplays. ð

ð¥ <b>A tournament is currently going on! Register via @eclregisbot</b> ð¥

Choose your mode: ð"
    keyboard = [[InlineKeyboardButton("Solo Game ð", callback_data="solo_game")], [InlineKeyboardButton("Team Game ð¥", callback_data="team_game")], [InlineKeyboardButton("Cancel â", callback_data="cancel")]]
...
async def create_team_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("state") != "TEAM_SETUP_HOST": 
        await update.message.reply_text("â No team game setup is active! Click 'Team Game' in /start first.")
        return
    if update.effective_user.id != game.get("host_id"):
        await update.message.reply_text("â Only the Game Host can create the teams!")
        return
    ...
    kb = [[InlineKeyboardButton("Join Team A ð´", callback_data="join_team_a"), InlineKeyboardButton("Join Team B ðµ", callback_data="join_team_b")]]
    context.job_queue.run_once(team_join_timeout, 10, data={"chat_id": chat_id}, name=f"team_join_{chat_id}")
    await update.message.reply_text("âï¸ <b>TEAM REGISTRATION OPEN!</b> âï¸

Players, choose your sides! You have 10 seconds to join. â³
...
async def changecap_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("mode") != "TEAM":
        await update.message.reply_text("â No active team match right now!")
        return
    if update.effective_user.id != game.get("host_id"):
        await update.message.reply_text("â Only the Game Host can change captains!")
        return
    if game.get("state") in ["TEAM_SETUP_HOST", "TEAM_JOINING", "TEAM_CAPTAINS"] and (not game.get("team_a", {}).get("captain") or not game.get("team_b", {}).get("captain")):
        await update.message.reply_text("â Cannot change captains before both teams have selected their initial captains!")
        return
    if not context.args:
    ...
    if team_choice not in ["a", "b"]:
        await update.message.reply_text("â Please specify team 'a' or 'b'. Example: /changecap a")
        return
    ...
    if not target_player:
        await update.message.reply_text(f"â User not found in Team {team_choice.upper()}! Make sure to reply to their message or tag them correctly.")
        return
    game[team_key]["captain"] = target_player["id"]
    await update.message.reply_text(f"â Team {team_choice.upper()} captain changed to {target_player['name']}!")
...
async def rejoin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("â³ <b>Registration Extended!</b> 30 more seconds to join the teams! ð¥", parse_mode="HTML")
...
async def changeover_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("mode") != "TEAM" or game.get("state") != "PLAYING":
        await update.message.reply_text("â No active team match is currently playing!")
        return
    if update.effective_user.id != game.get("host_id"):
        await update.message.reply_text("â Only the Game Host can change the number of overs!")
        return
    if game.get("innings") != 1:
        await update.message.reply_text("â You can only change the number of overs during the 1st innings!")
        return
    if not context.args or not context.args[0].isdigit():
        await update.message.reply_text("ð Usage: `/changeover [number]` (e.g., `/changeover 5`)", parse_mode="Markdown")
        return
    ...
    if new_overs <= played_overs:
        await update.message.reply_text(f"â The match has already crossed {played_overs} overs! The new target must be greater than {played_overs} overs.")
        return
        
    game["target_overs"] = new_overs
    await update.message.reply_text(f"â <b>Overs updated!</b> The match is now set for <b>{new_overs} overs</b> per side.", parse_mode="HTML")
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = context.bot_data.get(chat_id)
    if not game or game.get("mode") != "TEAM": 
        await update.message.reply_text("â No active team match setup found!")
        return
    if update.effective_user.id != game.get("host_id"):
        await update.message.reply_text("â Only the Game Host can add players manually!")
        return
    if not context.args:
    ...
    team_choice = context.args[0].lower()
    if team_choice not in ["a", "b"]:
        await update.message.reply_text("â Please specify team 'a' or 'b'. Example: /add a")
        return
    ...
    if not target_user:
        await update.message.reply_text("â Please reply to a user's message or make sure they have played before if using @username!")
        return
    if target_user.is_bot:
        await update.message.reply_text("â You cannot add a bot to the team!")
        return
    if is_user_playing_anywhere(context, target_user.id):
        await update.message.reply_text("â User is already in a game or in a queue in either this or other group")
        return
        
    in_a = any(p["id"] == target_user.id for p in game["team_a"]["players"])
    in_b = any(p["id"] == target_user.id for p in game["team_b"]["players"])
    if in_a:
        await update.message.reply_text(f"â {target_user.first_name} is already in Team A ð´!")
        return
    if in_b:
        await update.message.reply_text(f"â {target_user.first_name} is already in Team B ðµ!")
        return
    ...
    team_name = "TEAM A ð´" if team_choice == "a" else "TEAM B ðµ"
    await update.message.reply_text(f"â <b>{target_user.first_name}</b> has been manually added to {team_name} by the Host! ð¥", parse_mode="HTML")
    
    if game.get("is_paused_waiting_players"):
        if len(game["team_a"]["players"]) >= 2 and len(game["team_b"]["players"]) >= 2:
            game["is_paused_waiting_players"] = False
            await context.bot.send_message(chat_id, "â Minimum player requirement met! Resuming setup... â¶ï¸")
            await trigger_team_captains(context, chat_id, game)
...
async def remove_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("mode") != "TEAM": 
        ...
        return
    if update.effective_user.id != game.get("host_id"):
        await update.message.reply_text("â Only the Game Host can remove players manually!")
        return
    target_user, target_username = get_user_from_mention(update)
    if not target_user and not target_username:
        await update.message.reply_text("â Please reply to a user's message or tag their @username properly!")
        return
    ...
    if removed: await update.message.reply_text(f"â <b>{target_name}</b> has been successfully removed from their team! Numbers updated. ðª", parse_mode="HTML")
    else: await update.message.reply_text(f"â {target_user.first_name if target_user else target_username} is not in any team!")
...
async def changehost_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("mode") != "TEAM" or game.get("state") in ["NOT_PLAYING", None, "TEAM_FINISHED"]: 
        await update.message.reply_text("â No active team match to change host!")
        return
    ...
    if not (is_host or in_team_a or in_team_b):
        await update.message.reply_text("â ï¸ Warning: Only the Game Host or active players in Team A/B can use this command!")
        return
    ...
    if not target_user:
        await update.message.reply_text("â Please reply to a user's message or ensure they have played before if using @username!")
        return
    if target_user.is_bot:
        await update.message.reply_text("â A bot cannot be the Game Host!")
        return
        
    if is_host:
        game["host_id"] = target_user.id
        await update.message.reply_text(f"â Host privileges successfully transferred to <b>{target_user.first_name}</b>! ð", parse_mode="HTML")
    else:
        game["host_vote_target"] = target_user.id
        game["host_vote_name"] = target_user.first_name
        game["host_votes"] = set()
        kb = [[InlineKeyboardButton("Vote â (0/4)", callback_data="vote_host")]]
        await update.message.reply_text(f"ð³ï¸ Vote initiated to change host to <b>{target_user.first_name}</b>!
...
async def join_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("state") != "JOINING":
        await update.message.reply_text("No match is open for joining! Type /start âð")
        return
    user = update.effective_user
    if is_user_playing_anywhere(context, user.id):
        await update.message.reply_text("â you are already in a game or in a queue in either this or other group")
        return
    if any(p["id"] == user.id for p in game["players"]):
        await update.message.reply_text(f"â ï¸ <b>{user.first_name}</b>, you are ALREADY in the queue! Please wait for the match to start. â³ð§ââï¸", parse_mode="HTML")
        return
    ...
    if len(game["players"]) == 1:
        ...
        timer_msg = "
â³ <i>Auto-start timer initiated: Match begins in 70 seconds!</i>"
        
    await update.message.reply_text(f"â <b>{user.first_name}</b> joined! (Total: {len(game['players'])}) ð¥{timer_msg}", parse_mode="HTML")
...
async def leavesolo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.get("state") == "PLAYING":
        await update.message.reply_text("â The match has already started! You can't leave now.")
        return
    if game.get("state") == "JOINING":
        ...
        if player:
            game["players"] = [p for p in game["players"] if p["id"] != user_id]
            await update.message.reply_text(f"ð <b>{update.effective_user.first_name}</b> has left the queue. (Total: {len(game['players'])}) ð¥", parse_mode="HTML")
            if len(game["players"]) == 0:
                for prefix in ["autostart_", "queueremind_"]:
                    jobs = context.job_queue.get_jobs_by_name(f"{prefix}{chat_id}")
                    for job in jobs: job.schedule_removal()
                await update.message.reply_text("Queue is empty! ð Timer stopped.")
        else: await update.message.reply_text("You are not in the queue! â")
...
async def startsolo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update.effective_chat, update.effective_user.id):
        await update.message.reply_text("â Only group admins can start the match manually!")
        return
    game = context.bot_data.get(chat_id)
    if not game or game["state"] != "JOINING": return
    if len(game["players"]) < 2:
        await update.message.reply_text("We need at least 2 players! ð¥ð")
        return
    for prefix in ["autostart_", "queueremind_"]:
        jobs = context.job_queue.get_jobs_by_name(f"{prefix}{chat_id}")
        for job in jobs: job.schedule_removal()
...
async def endmatch_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private": return
    if not await is_admin(update.effective_chat, update.effective_user.id):
        await update.message.reply_text("â Only group admins can end the match!")
        return
    game = context.bot_data.get(chat_id)
    if not game or game.get("state") in ["NOT_PLAYING", None, "TEAM_FINISHED"]:
        await update.message.reply_text("â There is no active match to end!")
        return
    keyboard = [[InlineKeyboardButton("Yes, End Match â", callback_data=f"endmatch_yes_{chat_id}")], [InlineKeyboardButton("Cancel â", callback_data=f"endmatch_no_{chat_id}")]]
    await update.message.reply_text("â ï¸ <b>Admin Action:</b> Are you sure you want to force-end the current match?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
...
async def soloscore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("mode") != "SOLO" or game.get("state") in ["NOT_PLAYING", None]:
        await update.message.reply_text("â No active solo match is currently being played!")
        return
    await trigger_full_scorecard_message(context, chat_id, game)
...
async def teamscore_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game or game.get("mode") != "TEAM" or game.get("state") in ["NOT_PLAYING", None]:
        await update.message.reply_text("â No active team match is currently being played!")
        return
    await trigger_full_scorecard_message(context, chat_id, game)
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game = context.bot_data.get(chat_id)
    if not game or game.get("mode") != "TEAM": 
        await update.message.reply_text("â There is no active team match currently! This command is only for team matches.")
        return
    if game.get("state") != "PLAYING": 
        await update.message.reply_text("â The match hasn't started yet!")
        return
    
    if game.get("waiting_for") not in ["TEAM_OPENERS_BAT", "TEAM_BATTER_SELECT"]: 
        await update.message.reply_text("â Batters are already on the pitch! You cannot change them right now.")
        return

    batting_team = game["batting_team_ref"]
    if not context.args or not context.args[0].isdigit():
        text = "ð <b>AVAILABLE BATTERS:</b>
"
        for p in batting_team["players"]:
            status = "â (Out)" if p.get("is_out") else ("ð (On Pitch)" if p.get("is_striker") or p.get("is_non_striker") else "â (Available)")
            text += f"[{p.get('num', '?')}] {p['name']} - {status}
"
        text += "
ð <i>Usage: /batting [number] to select.</i>"
        await update.message.reply_text(text, parse_mode="HTML")
        return

    if update.effective_user.id not in [batting_team["captain"], game["host_id"]]: 
        await update.message.reply_text("â Only the Host or Batting Team Captain can select the batter!")
        return
    ...
    if not selected:
        await update.message.reply_text(f"â Player {p_num} not found in your team!")
        return
    if selected.get("is_out"):
        await update.message.reply_text(f"â {selected['name']} is already out! Select a different player.")
        return
    
    st = game.get("striker") or {}
    ns = game.get("non_striker") or {}
    if st.get("id") == selected["id"] or ns.get("id") == selected["id"]:
        await update.message.reply_text(f"â {selected['name']} is already on the pitch!")
        return
    ...
    if game["waiting_for"] == "TEAM_OPENERS_BAT":
        if not game.get("striker"):
            ...
            await update.message.reply_text(f"ð <b>{selected['name']}</b> selected as Striker!", parse_mode="HTML")
            ...
            caption_txt = f"ð <b>{selected['name']}</b> selected as Non-Striker!
...
Bowling Team Captain/Host, type /bowling to see bowlers or /bowling [num] to select opening bowler."
        ...
        await update.message.reply_text(f"ð <b>{selected['name']}</b> walks out to the pitch!", parse_mode="HTML")
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if game.get("waiting_for") in ["TEAM_OPENERS_BAT", "TEAM_BATTER_SELECT"]: 
        await update.message.reply_text("â Batters not selected yet warning! Let the batting team select their batter(s) first.")
        return
        
    if game.get("waiting_for") != "TEAM_BOWLER_SELECT": 
        await update.message.reply_text("â A bowler is already selected and bowling right now!")
        return
    
    bowling_team = game["bowling_team_ref"]
    if not context.args or not context.args[0].isdigit():
        text = "ð¥ <b>AVAILABLE BOWLERS:</b>
"
        for p in bowling_team["players"]:
            status = "â (Available)"
            if game.get("last_bowler_id") == p["id"]: status = "â³ (Bowled Last Over)"
            cb = game.get("current_bowler") or {}
            if cb.get("id") == p["id"]: status = "ð¥ (Bowling Now)"
            text += f"[{p.get('num', '?')}] {p['name']} - {p['balls_bowled']//6}.{p['balls_bowled']%6} Ov - {status}
"
        text += "
ð <i>Usage: /bowling [number] to select.</i>"
        await update.message.reply_text(text, parse_mode="HTML")
        return

    if update.effective_user.id not in [bowling_team["captain"], game["host_id"]]: 
        await update.message.reply_text("â Only the Host or Bowling Team Captain can select the bowler!")
        return
    ...
    if game.get("last_bowler_id") == selected["id"]:
        await update.message.reply_text("â A bowler cannot bowl two consecutive overs!")
        return
        
    game["current_bowler"] = selected
    game["waiting_for"] = "BOWLER"
    
    await update.message.reply_text(f"ð¥ <b>{selected['name']}</b> is handed the ball!", parse_mode="HTML")
    if game.get("innings_start_msg_pending"):
        game["innings_start_msg_pending"] = False
        await update.message.reply_text("ð¨ <b>THE INNINGS HAS BEGUN!</b>", parse_mode="HTML")
    await trigger_bowl(context, chat_id)
...
async def trigger_full_scorecard_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, game_data):
    ...
    if game_data.get("state") in ["NOT_PLAYING", "TEAM_FINISHED"]:
        bot_info = await context.bot.get_me()
        kb = [
            [InlineKeyboardButton("PLAY AGAIN ð", callback_data="play_again")], 
            [InlineKeyboardButton("ADD IN GROUP â", url=f"https://t.me/{bot_info.username}?startgroup=true")]
        ]
        markup = InlineKeyboardMarkup(kb)
        
    await context.bot.send_photo(chat_id=chat_id, photo=SCOREBOARD_IMG, caption=final_caption, parse_mode="HTML", reply_markup=markup)

async def send_top_performers_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, game):
    text = "ð <b>TOP PERFORMERS OF THE MATCH</b> ð
ââââââââââââââââââââââ
"
...
<b>{team_name}</b>
"
        text += f"ð <b>Best Batter:</b> {best_batter['name'][:15]} â <b>{best_batter['runs']}</b> ({best_batter['balls_faced']})
"
        b_ov, b_bl = divmod(best_bowler["balls_bowled"], 6)
        text += f"ð¥ <b>Best Bowler:</b> {best_bowler['name'][:15]} â <b>{best_bowler['wickets']}W</b> for {best_bowler['conceded']}R ({b_ov}.{b_bl} Ov)
"
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    free_hit_tag = "ð <b>FREE HIT ACTIVE!!</b>
" if game.get("is_free_hit") else ""
    
    dm_text = f"ð <b>Match in Progress!</b>

ð Batter: <b>{batter['name']}</b> ({batter['runs']} off {batter['balls_faced']})
ð¥ Over Status: {over_info}.

ð <b>Your Turn to Bowl!</b> Type a number from 1 to 6."
    keyboard = []
    if not game.get("special_used_this_over"): keyboard.append([InlineKeyboardButton("ð¯ Try for yorker ð¯", callback_data=f"special_{chat_id}")])
    ...
    if dm_sent:
        group_text = f"{free_hit_tag}ð <b>Status:</b>
ð <b>Batter:</b> {batter['name']} ({batter['runs']} off {batter['balls_faced']})
ð¥ <b>Bowler:</b> {bowler['name']} (Over: {over_info})

ð <a href='tg://user?id={bowler['id']}'>{bowler['name']}</a>, check your DM to bowl! ð¤«ð¥"
        group_kb = [[InlineKeyboardButton("Bowl Delivery ð¥", url=url)]]
    else:
...
â ï¸ <a href='tg://user?id={bowler['id']}'>{bowler['name']}</a>, I couldn't DM you! Click below to start me, then bowl! ð¤«ð¥"
        group_kb = [[InlineKeyboardButton("Start Bot & Bowl ð¤", url=fallback_url)]]
...
async def process_team_innings_end(context, chat_id, game):
    if game["innings"] == 1:
        ...
        text = f"ð <b>INNINGS BREAK! AB CHASE KARO !! </b> ð

ð¯ Target for the Bowling team: <b>{game['target']} runs</b> in {game['target_overs']} overs.
...
<code>/batting [number]</code> (do it twice)."
        ...
        team_b_score = game["team_b"]["score"]
        winning_team = None
        if team_a_score > team_b_score: winning_team = game["team_a"]["players"]
        elif team_b_score > team_a_score: winning_team = game["team_b"]["players"]
        if winning_team:
            for wp in winning_team: await update_user_db(wp["id"], {"exp": 40})
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if users_col is None:
        await msg.reply_text("â Database connection error.")
        return
    try:
        user_data = None
        ...
        if not user_data:
             ...
             await msg.reply_text(f"â Ek bhi match khela hai tune is bot se jo stats dekh raha ? {name}.")
             return
        ...
        level_name, req_exp, next_lvl = get_level_info(exp)
        ...
        stats_text = f"ð <b>{level_name} STATISTICS</b> ð
âââââââââââââââââââââââââââ
"
        stats_text += f"ð¤ <b>Name:</b> {user_data.get('first_name', 'Unknown')}
ð <b>ID:</b> <code>{user_data.get('user_id', 'Unknown')}</code>
"
        stats_text += f"ð <b>Current EXP:</b> {exp}
"
        if req_exp:
            stats_text += f"ð¯ <b>Next Level:</b> {next_lvl} (Need {req_exp - exp} more EXP)
"
        else:
            stats_text += f"ð¯ <b>Next Level:</b> MAXED OUT ð
"
        stats_text += f"âââââââââââââââââââââââââââ
"
        stats_text += f"ð <b>BATTING STATS</b>
ð¸ <b>Highest Score:</b> {hs_runs} ({hs_balls})
ð¸ <b>Total Runs:</b> {total_runs}
ð¸ <b>Batting Avg:</b> {avg:.2f} | <b>Strike Rate:</b> {sr:.2f}
"
        stats_text += f"ð¸ <b>6s:</b> {user_data.get('total_6s', 0)} | <b>4s:</b> {user_data.get('total_4s', 0)}
ð¸ <b>100s:</b> {user_data.get('centuries', 0)} | <b>50s:</b> {user_data.get('half_centuries', 0)}
"
        stats_text += f"ð¸ <b>Ducks ð¦:</b> {user_data.get('ducks', 0)}
âââââââââââââââââââââââââââ
"
        stats_text += f"ð¥ <b>BOWLING STATS</b>
ð¹ <b>Wickets:</b> {user_data.get('wickets', 0)}
ð¹ <b>Hat-Tricks:</b> {user_data.get('hat_tricks', 0)}
"
        stats_text += f"ð¹ <b>Overs Bowled:</b> {overs}.{rem_balls}
ð¹ <b>Economy:</b> {eco:.2f}
âââââââââââââââââââââââââââ
"
        stats_text += f"ð <b>MATCH & AWARDS</b>
ð¸ <b>Solo Matches:</b> {user_data.get('solo_matches', 0)}
ð¸ <b>Team Matches:</b> {user_data.get('team_matches', 0)}
"
        stats_text += f"ð¸ <b>MOTM Awards:</b> {user_data.get('motm', 0)}
âââââââââââââââââââââââââââ"
        ...
        await msg.reply_photo(photo=stats_img, caption=stats_text, parse_mode="HTML")
         ...
         await msg.reply_text("â An error occurred while fetching stats.")

async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = [
        [InlineKeyboardButton("WEEKLY LEADERBOARD ð", callback_data="lb_weekly")],
        [InlineKeyboardButton("LIFETIME LEADERBOARD ð", callback_data="lb_lifetime")]
    ]
    await update.message.reply_text("ð <b>View our top performers!</b>
Select a leaderboard below:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")

async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in OWNER_IDS:
        await update.message.reply_text("â You ain't the owner of this bot biiichhh.")
        return
    ...
    if chats_col is None:
        ...
        return
    success = 0; failed = 0
    status_msg = await update.message.reply_text("Broadcasting started... â³")
    ...
    await status_msg.edit_text(f"â <b>Broadcast finished!</b>

ð¨ Sent: {success}
â Failed: {failed}", parse_mode="HTML")
...
async def botstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"ð <b>Bot Statistics</b>

ð¤ Total Users Interacted: {users_count}
ð¥ Total Groups Present: {groups_count}", parse_mode="HTML")
...
async def botgroups_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"ð <b>Bot Groups ({len(groups)}):</b>
...
async def spamfree_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private": return
    game = context.bot_data.get(chat_id)
    if not game or game.get("state") != "TEAM_SPAMFREE_WAIT": return
    if update.effective_user.id != game.get("host_id"):
        await update.message.reply_text("â Only the Game Host can use the spamfree command!")
        return
    
    jobs = context.job_queue.get_jobs_by_name(f"spamfree_{chat_id}")
    for job in jobs: job.schedule_removal()
    
    game["spamfree"] = True
    game["state"] = "PLAYING"
    await update.message.reply_text("ð¡ï¸ <b>SPAM-FREE MODE ACTIVATED!</b> Bowlers cannot bowl the same delivery more than twice in a row.
...
async def spamfree_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    game["spamfree"] = False
    game["state"] = "PLAYING"
    await context.bot.send_message(chat_id, "â³ Time is up! â ï¸ <b>SPAM IS ALLOWED.</b>
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "solo_game":
        if game.get("state") not in ["NOT_PLAYING", None, "TEAM_FINISHED"]:
            try: await query.edit_message_caption(caption="â A match is already active or setting up in this group!", reply_markup=None)
            except: pass
            return
        keyboard = [[InlineKeyboardButton("3 Balls ð¥", callback_data="spell_3")], [InlineKeyboardButton("6 Balls ð¥", callback_data="spell_6")]]
        try: await query.message.delete()
        except: pass
        await context.bot.send_photo(chat_id=chat_id, photo="https://res.cloudinary.com/dxgfxfoog/image/upload/v1777720022/file_00000000483072079f73014e1bba1fde_l4thrv.png", caption="Select Spell Limit: âï¸ð", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        if game.get("state") not in ["NOT_PLAYING", None, "TEAM_FINISHED"]:
            ...
            return
        text = "ð¥ <b>TEAM GAME MODE</b> ð¥

Form two teams, appoint captains, toss the coin, and clash in an epic T20-style showdown! ðð

Who will take charge?"
        kb = [[InlineKeyboardButton("HOST BANUNGA ð¿", callback_data="host_banunga")], [InlineKeyboardButton("CANCEL â", callback_data="cancel")]]
        try: await query.message.delete()
        except: pass
        ...
        if is_user_playing_anywhere(context, user_id):
            try: await query.answer("â you are already in a game or in a queue in either this or other group", show_alert=True)
            except: await context.bot.send_message(chat_id, "â you are already in a game or in a queue in either this or other group")
            return
        context.bot_data[chat_id] = {"state": "TEAM_SETUP_HOST", "host_id": user_id, "mode": "TEAM"}
        try: await query.edit_message_caption(caption=f"ð <a href='tg://user?id={user_id}'>{update.effective_user.first_name}</a> is the Game Host!
...
Host, please send /create_team to open the team registration.", parse_mode="HTML", reply_markup=None)
        ...
        if is_user_playing_anywhere(context, user_id):
            await context.bot.send_message(chat_id, "â you are already in a game or in a queue in either this or other group")
            return
        in_a = any(p["id"] == user_id for p in game["team_a"]["players"])
        in_b = any(p["id"] == user_id for p in game["team_b"]["players"])
        if in_a or in_b:
            team_name = "Team A ð´" if in_a else "Team B ðµ"
            try: await query.answer(f"â ï¸ You are already in {team_name}! Wait for the host to start.", show_alert=True)
            except: pass
            return
        ...
        await context.bot.send_message(chat_id, f"ð´ <b>{update.effective_user.first_name}</b> joined Team A!", parse_mode="HTML")
        ...
        await context.bot.send_message(chat_id, f"ðµ <b>{update.effective_user.first_name}</b> joined Team B!", parse_mode="HTML")
        if game.get("is_paused_waiting_players") and len(game["team_a"]["players"]) >= 2 and len(game["team_b"]["players"]) >= 2:
            game["is_paused_waiting_players"] = False
            await context.bot.send_message(chat_id, "â Minimum player requirement met! Resuming setup... â¶ï¸")
            await trigger_team_captains(context, chat_id, game)
        ...
        await context.bot.send_message(chat_id, f"ð <b>{update.effective_user.first_name}</b> is now Captain of {'Team A ð´' if team_key == 'team_a' else 'Team B ðµ'}!", parse_mode="HTML")
        if game["team_a"]["captain"] and game["team_b"]["captain"]:
            ...
            kb = [[InlineKeyboardButton("Heads ðª", callback_data="toss_heads"), InlineKeyboardButton("Tails ðª", callback_data="toss_tails")]]
            toss_vid = "https://res.cloudinary.com/dxgfxfoog/video/upload/v1777819028/VID_20260503195638_lhif0h.mp4"
            caption_msg = f"ðª <b>TOSS TIME!</b>
<a href='tg://user?id={cap_id}'>{cap_name}</a>, call the toss!"
        ...
        if won_toss:
            ...
            winner_team_name = "Team A ð´" if game["toss_winner_team"] == "team_a" else "Team B ðµ"
            kb = [[InlineKeyboardButton("Bat ð", callback_data="toss_bat"), InlineKeyboardButton("Bowl ð¥", callback_data="toss_bowl")]]
            try: await query.message.delete()
            except Exception: pass
            await context.bot.send_message(chat_id, text=f"ð <b>{winner_team_name}</b> won the toss! What will you do?", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
        else:
            game["state"] = "TEAM_TOSS_DECISION"
            ...
            await context.bot.send_message(chat_id, text=f"â You lost the toss!

ð <b>{winner_team_name}</b> (<a href='tg://user?id={cap_id}'>{cap_name}</a>) won the toss. What will they choose?", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
        ...
        await context.bot.send_message(chat_id, text=f"â The captain chose to {dec_text} first!")
        ...
        try: await query.edit_message_text(f"â Match set for <b>{overs} Overs</b> per side!", parse_mode="HTML", reply_markup=None)
        except: pass
        ...
        await context.bot.send_message(
            chat_id, 
            f"â ï¸ <a href='tg://user?id={host_id}'>{host_name}</a>, you can make this game spam-free by clicking on /spamfree
...
"
        ...
        try: await query.edit_message_caption(caption=f"ð <b>Queue Open!</b> (Spell: {spell_len} balls) âï¸
ð Type /join
ð Type /leavesolo to exit queue
ð Admin can type /startsolo", parse_mode="HTML", reply_markup=None)
        except: pass

    elif query.data == "cancel":
        if game.get("state") == "PLAYING":
            try: await query.edit_message_caption(caption="â Match is already playing! Use /endmatch to stop it.", reply_markup=None)
            except: pass
            return
        game["state"] = "NOT_PLAYING"
        ...
        try: await query.edit_message_caption(caption="Setup cancelled. ðâ", reply_markup=None)
        except: pass
        ...
        if votes >= 4:
            ...
            try: await query.edit_message_text(f"â Vote passed! Game Host successfully changed to <b>{game['host_vote_name']}</b>! ð", parse_mode="HTML", reply_markup=None)
            except: pass
        else:
            try: await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"Vote â ({votes}/4)", callback_data="vote_host")]]))
            except: pass
        ...
        if not await is_admin(update.effective_chat, update.effective_user.id):
            await context.bot.send_message(chat_id, "â Only admins can click this!")
            return
        if action == "yes":
            ...
            try: await query.edit_message_text("ð <b>Match has been force-ended by an Admin.</b>", parse_mode="HTML", reply_markup=None)
            except Exception: pass
        elif action == "no":
            try: await query.edit_message_text("â Force-end cancelled. The match continues!", reply_markup=None)
            except Exception: pass
        ...
        if roll <= 60:
            ...
            await context.bot.send_message(group_id, "ð¨ <b>WIDE BALL!</b> 1 extra run. Bowler must re-bowl! ð¥", parse_mode="HTML")
...
Koi na kismat ki baat hai !", parse_mode="HTML", reply_markup=None)
            ...
            await send_media_safely(context, group_id, MEDIA["batter_turn"], f"ð¨ Ball delivered !! ð¥ð¨
ð <a href='tg://user?id={batter['id']}'>{batter['name']}</a>, type {hit_opts} to hit! ðð")
            set_afk_timer(context, group_id, batter["id"], "BATTER")
        else:
            msg = "ð¯ <b>Yorker pel diya bhai ð¶âð«ï¸</b> Let's see how the batter reacts...
â ï¸ If the batter chooses "
            msg += "0-3, they survive. " if game.get("mode") == "TEAM" else "1-3, they survive. "
            msg += "Otherwise, they are OUT! âï¸"
            try: await query.edit_message_text(msg, parse_mode="HTML", reply_markup=None)
            except: pass
            game["current_bowl"] = "YORKER"
            game["waiting_for"] = "BATTER"
            hit_opts = "1-6" if game.get("mode") == "SOLO" else "0-6"
            await send_media_safely(context, group_id, MEDIA["batter_turn"], f"ð¨ Ball bowled! ð¥ð¨
ð <a href='tg://user?id={batter['id']}'>{batter['name']}</a>, type {hit_opts} to hit! ðð")
            set_afk_timer(context, group_id, batter["id"], "BATTER")
        ...
        pipeline_bat = [
            ...
            {"$addFields": {"sr": {"$cond": [{"$gt": [f"${bf_field}", 0]}, {"$multiply": [{"$divide": [f"${run_field}", f"${bf_field}"]}, 100]}, 0]}}},
        ...
        if is_weekly and not top_batters and not top_bowlers:
            try: await query.edit_message_text("â³ <b>Still fetching data...</b> Play some matches to get on the board!", parse_mode="HTML")
            except: pass
            return
            
        text = f"ð <b>{'WEEKLY' if is_weekly else 'LIFETIME'} LEADERBOARD</b> ð

"
        text += "ð <b>TOP 5 BATTERS</b>
"
...
"
        ...
        kb = [[InlineKeyboardButton("Back ð", callback_data="lb_back")]]
        ...
        try: await query.edit_message_text("ð <b>View our top performers!</b>
Select a leaderboard below:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
        except: pass
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        ...
        if val < 1 or val > 6:
            await update.message.reply_text("â Bowlers can only bowl numbers from 1 to 6!")
            return
        ...
        if game.get("mode") == "TEAM" and game.get("spamfree"):
            ...
            if len(last_balls) >= 2 and last_balls[-1] == val and last_balls[-2] == val:
                await update.message.reply_text("â ï¸ <b>SPAM FREE MODE ACTIVE:</b> You cannot bowl the same delivery more than 2 times in a row! Choose a different delivery.", parse_mode="HTML")
                return
                
            bowler["last_balls"] = last_balls + [val]
            if len(bowler["last_balls"]) > 2:
                bowler["last_balls"] = bowler["last_balls"][-2:]
        ...
        kb = [[InlineKeyboardButton("Back to Game ð", url=chat_url)]] if chat_url else []
        hit_opts = "1-6" if game.get("mode") == "SOLO" else "0-6"
        await update.message.reply_text(f"Choice locked! ð You bowled a <b>{val}</b>.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb) if kb else None)
        await send_media_safely(context, group_id, MEDIA["batter_turn"], f"ð¨ Ball bowled! ð¥ð¨
ð <a href='tg://user?id={batter['id']}'>{batter['name']}</a>, type {hit_opts} to hit! ðð")
        set_afk_timer(context, group_id, batter["id"], "BATTER")
    else:
        ...
        if bowl_val == "NO_BALL":
            ...
            result_text = f"ð¨ <b>IT WAS A NO BALL!</b> 1 penalty run.
ð <b>NEXT BALL WILL BE A FREE HIT!</b> ð

ð Batter hit: <b>{hit_val}</b>

"
            if hit_val == 0: result_text += f"ð¡ï¸ <b>Solid defense! Dot ball.</b> ({batter['name']}: {batter['runs']} off {batter['balls_faced']})"
            else: result_text += f"ðââï¸ <b>Great shot! {hit_val} runs!</b> ð¥ ({batter['name']}: {batter['runs']} off {batter['balls_faced']})"
            ...
            if old_runs < 100 and batter["runs"] >= 100:
                ...
                milestone_text = f"ð <b>CENTURY! TAKE A BOW!</b> ð¯ð¥
<a href='tg://user?id={batter['id']}'>{batter['name']}</a> has smashed a glorious century! What an innings! ððï¸"
            elif old_runs < 50 and batter["runs"] >= 50:
                await update_user_db(batter["id"], {"exp": 50})
                milestone_media = MEDIA["50"]
                milestone_text = f"ð <b>HALF-CENTURY! BRILLIANT INNINGS!</b> ð¥ð
<a href='tg://user?id={batter['id']}'>{batter['name']}</a> reaches 50! Keep it going! ð¥"
            ...
            if game.get("mode") == "TEAM" and hit_val % 2 != 0:
                swap_strike(game)
                await context.bot.send_message(chat_id, f"ð Strike rotated! ð <a href='tg://user?id={game['striker']['id']}'>{game['striker']['name']}</a> is now on strike!", parse_mode="HTML")
            ...
            if not survives:
                if is_free_hit:
                    game["is_free_hit"] = False
                    bowler["consecutive_wickets"] = 0
                    result_text = f"ð¥ Bowler delivery: <b>YORKER</b>
ð Batter hit: <b>{hit_val}</b>

ð¥ <b>BOWLED! BUT IT'S A FREE HIT!</b> ð
...
ð¥ <b>HOWZAT! OUT!</b> âï¸ {batter['name']} is bowled by a lethal yorker for {batter['runs']}! ðð¶ââï¸"
                    await send_media_safely(context, chat_id, MEDIA["yorker"], result_text, reply_to_message_id=update.message.message_id)
                    if batter["runs"] == 0: await send_media_safely(context, chat_id, MEDIA["duck"], f"ð¦ <a href='tg://user?id={batter['id']}'>{batter['name']}</a> got a duck ð¦")
                    ...
                    if bowler["consecutive_wickets"] == 3:
                        ...
                        await send_media_safely(context, chat_id, ht_vid, f"ð© <b>HAT-TRICK!</b> <a href='tg://user?id={bowler['id']}'>{bowler['name']}</a>, you are a magician!! ðªð¥")
                    ...
                    if game.get("mode") == "TEAM":
                        ...
                        if game["batting_team_ref"]["wickets"] >= len(game["batting_team_ref"]["players"]) - 1:
                            ...
                            game["waiting_for"] = "TEAM_BATTER_SELECT"
                            await context.bot.send_message(chat_id, f"ð Captain/Host, type <code>/batting</code> to see batters list or <code>/batting [number]</code> to select the next batter.", parse_mode="HTML")
                    else:
                        game["batter_idx"] += 1
                        if game["batter_idx"] >= len(game["players"]):
                            await check_solo_winner_exp(game)
...
ðââï¸ <b>Great shot! Dug out the yorker for {hit_val} runs!</b> ð¥ ({batter['name']}: {batter['runs']} off {batter['balls_faced']})"
                ...
                if old_runs < 100 and batter["runs"] >= 100:
                    await update_user_db(batter["id"], {"exp": 150})
                    await send_media_safely(context, chat_id, MEDIA["100"], f"ð <b>CENTURY! TAKE A BOW!</b> ð¯ð¥
<a href='tg://user?id={batter['id']}'>{batter['name']}</a> has smashed a glorious century! What an innings! ððï¸")
                elif old_runs < 50 and batter["runs"] >= 50:
                    await update_user_db(batter["id"], {"exp": 50})
                    await send_media_safely(context, chat_id, MEDIA["50"], f"ð <b>HALF-CENTURY! BRILLIANT INNINGS!</b> ð¥ð
<a href='tg://user?id={batter['id']}'>{batter['name']}</a> reaches 50! Keep it going! ð¥")
            ...
            if is_free_hit:
                game["is_free_hit"] = False
                bowler["consecutive_wickets"] = 0
                result_text = f"ð¥ Bowler delivery: <b>{bowl_val}</b>
ð Batter hit: <b>{hit_val}</b>
...
ð¥ <b>HOWZAT! OUT!</b> âï¸ {batter['name']} is dismissed for {batter['runs']}! ðð¤¸ð»
{batter['name']} KOI NA HOTA HAI !! "
            ...
            result_text = f"ð Batter hit: <b>{hit_val}</b>
...
<a href='tg://user?id={batter['id']}'>{batter['name']}</a> reaches 50! Keep it going! ð¥")
        ...
        if is_over_complete:
            spell_text = f"ð <b>Over Completed!</b> ð {bowler['name']} finished.
"
            if game.get("mode") == "TEAM":
                swap_strike(game)
                ...
                spell_text += f"
ð Score: {team['score']}/{team['wickets']}
"
                
                if game.get("striker"):
                    spell_text += f"ð Strike rotated for new over! ð <a href='tg://user?id={game['striker']['id']}'>{game['striker']['name']}</a> is now on strike!
"
