...
def commit_player_stats(game):
    ...
    for p in players:
        ...
        update_highest_score(p["id"], runs, balls_faced)
        updates = {
            "total_runs": runs,
        ...
        if runs >= 100:
            updates["centuries"] = 1
            updates["half_centuries"] = 1 # FIX: Update half-centuries too
        elif runs >= 50:
...
async def is_admin(chat, user_id):
    try:
        ...
        print(f"Admin Check Error (get_administrators): {e}")
        try:
            member = await chat.get_member(user_id)
            return member.status in ["administrator", "creator"]
        except Exception as e2:
            print(f"Admin Check Error (get_member): {e2}")
            return False
...
def generate_scorecard(game):
    ...
    for p in game["players"]:
        ...
        text += f"👤 <b>{p['name']}</b>\n"
        text += f"  🏏 Bat: <b>{p['runs']}</b> ({p['balls_faced']})\n"
...
def generate_team_scorecard(game):
    ...
    if game.get("innings") == 2 and game.get("state") != "TEAM_FINISHED":
        ...
        text += f"🎯 <b>Target:</b> {target} | <b>Need {max(0, runs_needed)} runs in {balls_rem} balls.</b>\n"
        text += "━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    for team_key, team_name in [("team_a", "🔴 TEAM A"), ("team_b", "🔵 TEAM B")]:
        ...
        if opp_team:
            played_overs, played_balls = divmod(opp_team["balls_bowled"], 6)
        else:
            played_overs, played_balls = 0, 0
            
        text += f"🎖️ <b>{team_name}</b> ➜ <b>{team['score']}/{team['wickets']}</b> <i>({played_overs}.{played_balls} Ov)</i>\n"
        
        batters_txt = ""
        for p in team["players"]:
            if p["balls_faced"] > 0 or p.get("is_striker") or p.get("is_non_striker") or p.get("is_out"):
                status = "❌" if p.get("is_out") else "🏏"
                batters_txt += f"  {status} {p['name']} ➜ {p['runs']} ({p['balls_faced']})\n"
        if batters_txt:
            text += f"<i>Batters:</i>\n{batters_txt}"
        ...
        for p in team["players"]:
            if p["balls_bowled"] > 0:
                ...
                bowlers_txt += f"  🥎 {p['name']} ➜ {p_ov}.{p_bl} Ov | {p['conceded']}R | {p['wickets']}W | Eco {eco:.1f}\n"
        if bowlers_txt:
            text += f"<i>Bowlers:</i>\n{bowlers_txt}"
        text += "━━━━━━━━━━━━━━━━━━━━━━\n"
...
async def add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_user.id != game.get("host_id"): # STRICT HOST CHECK MAINTAINED
    ...
    if not target_user and target_username:
        if users_col is not None:
            db_user = users_col.find_one({"username": target_username})
            if db_user:
                class DummyUser:
...
async def batting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Warn if batters are already on pitch
...
async def bowling_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Warn if bowler is already selected
...
async def trigger_bowl(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    # FIX: Adding balls faced in brackets
    dm_text = f"🏏 <b>Match in Progress!</b>\n\n"
    dm_text += f"🏏 Batter: <b>{batter['name']}</b> ({batter['runs']} off {batter['balls_faced']})\n"
...
async def userstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Removed the thunder delay message completely to respond instantly.
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "solo_game":
        ...
        if action == "yes":
            ...
            if game_ref: 
                try: commit_player_stats(game_ref)
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        ...
        # FIX: Instantly lock the state to prevent spamming! No more thunder emojis.
        ...
        if bowl_val == "NO_BALL":
            ...
            if not survives:
                ...
                result_text = f"🥎 Bowler delivery: <b>YORKER</b>\n🏏 Batter hit: <b>{hit_val}</b>\n\n"
                result_text += f"🏃‍♂️ <b>Great shot! Dug out the yorker for {hit_val} runs!</b> 🔥 ({batter['name']}: {batter['runs']} off {batter['balls_faced']})"
                media_url = MEDIA.get(hit_val, MEDIA[0])
        ...
        if is_over_complete:
            ...
            if game.get("mode") == "TEAM":
                ...
                game["special_used_this_over"] = False # FIX: Reset special move for TEAM MODE here
            ...
            if game["state"] == "PLAYING" and game.get("waiting_for") == "PROCESSING_BATTER":
                game["waiting_for"] = "BOWLER"
                
        # FIX: Delayed Bowler response by 0.4 seconds
