...
BATTING_COMMENTARY = [
    ...
    "💥 Smashed it! Pure class from the batter!",
    "🏏 Textbook cricket! Timing was perfect!",
    "⚡ Explosive hitting! The fielders had no chance!",
    "🎯 Right in the slot and the batter pounced on it!",
    "🌟 Brilliant execution! The batter is in the zone!",
    "🚀 Sent into orbit! Phenomenal power!",
    "👑 That's why they call them the BATTER! Absolute class!",
    "🔥 The wrists! The footwork! Pure artistry!",
    "💫 Batter is on fire today! Bowler in trouble!",
    "🎪 What a display of batting! A master at work!",
    "⚡ Lightning fast bat speed — the bowler didn't stand a chance!",
    "🏆 That's a statement shot! Batter means business!",
    "🎶 Cricket poetry in motion! Gorgeous stroke play!",
    "😤 Batter said: NOT TODAY BOWLER! Massive hit!",
    "🌪️ The ball is still travelling! What a strike!",
    "👏 The whole ground on their feet for that one!",
    "🔥 ICE in the veins! Batter keeps their cool and delivers!",
    "💎 That's a gem of a shot! Straight from the textbook!",
    "🎯 Picked the gap perfectly! Smart cricket!",
]
...
def generate_scorecard(game):
    ...
    for p in game.get("players", []):
        ...
        text += (
            f"👤 <b>{p['name']}</b>➜ \n"
            f"🏏 Bat:\n<b>{p.get('runs', 0)}</b> ({p.get('balls_faced', 0)})\n"
            f"🥎 Bowl:\n<b>{p.get('wickets', 0)}</b>W | "
    ...
    if query.data == "solo_game":
        ...
        kb = [
            ...
            [
                ...
                InlineKeyboardButton("🏃 Most Runs",       callback_data="rank_runs"),
            ],
            [
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bowl_val == "NO_BALL":
        ...
        if hit_val > 0:
            await context.bot.send_message(chat_id, random.choice(BATTING_COMMENTARY), parse_mode="HTML")
...
async def _send_sr_ranking(query):
    if users_col is None:
        await query.edit_message_text("❌ Database not connected.")
        return
    docs = await users_col.find({"balls_faced": {"$gt": 20}}).to_list(length=50)
    random.shuffle(docs)
