...
WEBHOOK_URL = os.getenv("WEBHOOK_URL") # e.g., https://your-railway-app.up.railway.app
    ...
    if chats_col is not None and update.effective_chat:
        try:
            chats_col.update_one(
                ...
                {"$set": {"chat_id": update.effective_chat.id, "type": update.effective_chat.type}},
                upsert=True
            )
        except Exception:
            pass
...
async def send_media_safely(context, chat_id, media_url, caption, reply_markup=None, reply_to_message_id=None):
    try:
        # Added timeouts to prevent the bot from getting stuck if Giphy/Cloudinary takes too long to respond
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat in chats:
        ...
        try:
            if message_to_send:
                await context.bot.copy_message(chat_id=chat_id, from_chat_id=update.effective_chat.id, message_id=message_to_send.message_id)
            else:
...
async def botstats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users_count = chats_col.count_documents({"type": "private"})
